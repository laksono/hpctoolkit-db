 ...
 */
  ...
  #if UPCRI_SYMMETRIC_SEGMENTS && !UPCRI_USING_GCCUPC
 ...
 */
  ...
  #endif
  #if UPCRI_USING_GCCUPC && UPCRI_SPTR_ADDR_FIRST
...
_upcr_local_to_shared_ref(void *lptr, upcr_shared_ptr_t *result UPCRI_PT_ARG) 
{
    ...
    *result = (((uint64_t) (((uintptr_t)lptr) UPCRI_MINUS_MY_OFFSET)) << UPCRI_ADDR_SHIFT)
    ...
    result->s_phase = 0;
  #endif
  upcri_checkvalid_shared(*result);
}
...
_upcr_local_to_pshared_ref(void *lptr, upcr_pshared_ptr_t *result UPCRI_PT_ARG) 
{
    ...
    *result = (((uint64_t) (((uintptr_t)lptr) UPCRI_MINUS_MY_OFFSET)) << UPCRI_ADDR_SHIFT)
    ...
    result->s_thread = upcr_mythread();
  #endif
  upcri_checkvalid_pshared(*result);
}
		...
		size_t blockelems UPCRI_PT_ARG)
{
    ...
    ptrdiff_t thread_incr;
    size_t numthreads = upcr_threads();
    uintptr_t const sptr_addr =  upcr_addrfield_shared(sptr);
    upcr_thread_t const sptr_thread = upcr_threadof_shared(sptr);
    upcr_phase_t const sptr_phase = upcr_phaseof_shared(sptr);
    ssize_t block_incr;
    ssize_t elemaddr_incr;
    long const phase_plus = sptr_phase + inc;
	...
	ssize_t numer, denom; /* Both should be signed types */
    ...
    elemaddr_incr = ((ssize_t)out_phase - sptr_phase) + (block_incr*blockelems);
    out_addr = sptr_addr + (elemaddr_incr*(ssize_t)elemsz);
...
{
  upcri_checkvalid_pshared(*psptr);
  upcri_assert(elemsz > 0);
  #if UPCRI_SYMMETRIC_PSHARED
...
_upcr_inc_pshared1(upcr_pshared_ptr_t *psptr, size_t elemsz, ptrdiff_t inc UPCRI_TV_ARG)
{
    ...
    int modoff = (inc >= 0) ? 0 : 1 - numthreads;
    ptrdiff_t tsum = upcr_threadof_pshared(*psptr) + inc + modoff;
    uintptr_t addr = upcr_addrfield_pshared(*psptr);
...
_upcr_add_pshared1(upcr_pshared_ptr_t sptr, size_t elemsz, ptrdiff_t inc UPCRI_TV_ARG)
{
    ...
    ptrdiff_t block_incr;
    int modoff = (inc >= 0) ? 0 : 1 - numthreads;
    ptrdiff_t tsum = upcr_threadof_pshared(sptr) + inc + modoff;
    uintptr_t addr = upcr_addrfield_pshared(sptr);
    upcr_thread_t threadid;
...
{
  upcri_checkvalid_shared(ptr1);
  upcri_checkvalid_shared(ptr2);
  #if UPCRI_PACKED_SPTR
...
{
    /* use unsigned values to avoid errors with negatives values */
    ...
    upcri_checkvalid_shared(sptr1);
    upcri_checkvalid_shared(sptr2);
    upcri_assert(elemsz > 0);

    return (((addr1-addr2)/elem + (phase2-phase1))*numthreads) 
		  ...
		  size_t elemsz)
{
  ...
  upcri_checkvalid_pshared(sptr1);
  upcri_checkvalid_pshared(sptr2);
  upcri_assert(elemsz > 0);
