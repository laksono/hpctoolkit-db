 ...
 * $Id: upcr_alloc.c,v 1.98 2012/03/05 18:41:13 phargrov Exp $
 *
 */
...
typedef struct {
  upcri_alloccaller_t caller;
  int islocalheap;
} upcri_umalloc_context_t;
GASNETT_INLINE(upcri_umalloc_context)
upcri_umalloc_context_t upcri_umalloc_context(upcri_alloccaller_t caller, int islocalheap) {
  upcri_umalloc_context_t retval;
  retval.caller = caller;
  retval.islocalheap = islocalheap;
  return retval;
}
#if UPCR_DEBUG
  GASNETT_THREADKEY_DEFINE(_upcri_umalloc_context_key);
  #define upcri_set_umalloc_context(context) do {                       \
      upcri_umalloc_context_t *mycontext =                              \
        gasnett_threadkey_get(_upcri_umalloc_context_key);              \
      if_pf (mycontext == NULL) {                                       \
        mycontext = upcri_checkmalloc(sizeof(upcri_umalloc_context_t)); \
        gasnett_threadkey_set(_upcri_umalloc_context_key, mycontext);   \
      }                                                                 \
      *mycontext = (context);                                           \
    } while (0)
  #define upcri_get_umalloc_context(context) do {          \
      upcri_umalloc_context_t *mycontext =                 \
        gasnett_threadkey_get(_upcri_umalloc_context_key); \
      upcri_assert(mycontext);                             \
      (context) = *mycontext;                              \
    } while (0)
  extern void upcri_umalloc_corruption_callback(void) {
    /* called by umalloc in debug mode when it detects heap corruption */
    upcri_umalloc_context_t context;
    upcri_get_umalloc_context(context);
    upcri_err("Thread %i detected memory corruption or an illegal operation\n"
              " on the UPC %s shared heap, while performing a %s operation",
      (int)UPCRI_ALLOCCALLER_THREAD(context.caller),
      (context.islocalheap?"local":"global"),
      UPCRI_ALLOCCALLER_FN(context.caller)
    );
  }
#else
  #define upcri_set_umalloc_context(context)
  #define upcri_get_umalloc_context(context)
#endif
...
/* perform a local_alloc on a given local pthread's sharedlocal heap, 
 ...
 */
upcr_shared_ptr_t 
upcri_local_alloc(size_t nblocks, size_t blocksz, upcri_pthread_t pthread,
                  upcri_alloccaller_t caller)
{
    void * ptr;
    size_t prevgrowth = 0;
    size_t allocsz = nblocks * blocksz;
    upcri_localheap_t *lh = localheap(pthread);
    upcri_umalloc_context_t context = upcri_umalloc_context(caller, 1);
    ...
    upcri_set_umalloc_context(context);
...
#if UPCR_DEBUG /* Debug output in case bug 2505 pops up again */
    ...
    gasnet_hsl_unlock(&lh->lock);
    return upcr_local_to_shared_withphase(ptr, 0, 
				upcri_1stthread(gasnet_mynode()) + pthread);
}

upcr_shared_ptr_t 
_upcr_local_alloc(size_t nblocks, size_t blocksz UPCRI_PT_ARG)
{
  ...
  retval = upcri_local_alloc(nblocks, blocksz, upcri_mypthread(),
         UPCRI_ALLOCCALLER(UPCRI_ALLOCCALLERFN_LOCAL, upcr_mythread()));

  #define UPCRI_PEVT_ARGS , (nblocks*blocksz), &retval
  ...
  #undef UPCRI_PEVT_ARGS
    ...
    { char ptrstr[UPCRI_DUMP_MIN_LENGTH];
      ...
      UPCRI_TRACE_PRINTF(("HEAPOP upc_alloc(%llu) => %s", 
          (unsigned long long)(nblocks * blocksz), ptrstr));
    }   
  #endif
  return retval;
}
...
{
    gasnet_hsl_lock(&heapsizes_lock);
    largest_local_heapsize = (uintptr_t) newlargestlh;
    gasnet_hsl_unlock(&heapsizes_lock);
		       ...
		       upcri_alloccaller_t caller)
{
    void * newaddr;
    size_t morebytes = 0;
    size_t allocsz = blocksz * upcri_ceiling_div(nblocks, upcr_threads()); 
    upcri_umalloc_context_t context = upcri_umalloc_context(caller, 0);
     ...
     */
    if (nblocks == 1) {
	*pout = upcri_local_alloc(nblocks, blocksz, 0, caller);
	return;
    }
    ...
    upcr_local_to_shared_ref_withphase(newaddr, 0, 0, pout);
}
...
upcri_do_local_free(upcr_shared_ptr_t sptr)
{
	...
	upcri_localheap_t *h = localheap(pthread);
        upcri_umalloc_context_t context = 
           upcri_umalloc_context(UPCRI_ALLOCCALLER(UPCRI_ALLOCCALLERFN_FREE, thread), 1);
	gasnet_hsl_lock(&h->lock);
        upcri_set_umalloc_context(context);
	upcra_sharedlocal_free(h->heap, addr);
	gasnet_hsl_unlock(&h->lock);
    } else if (upcri_ptr_within_global_heap(addr)) {
	if (gasnet_mynode() == 0) {
            upcri_umalloc_context_t context = 
              upcri_umalloc_context(UPCRI_ALLOCCALLER(UPCRI_ALLOCCALLERFN_FREE, thread), 0);
	    gasnet_hsl_lock(&global_heap.lock);
            upcri_set_umalloc_context(context);
	    upcra_sharedglobal_free(global_heap.heap, addr);
	    gasnet_hsl_unlock(&global_heap.lock);
	} else {
