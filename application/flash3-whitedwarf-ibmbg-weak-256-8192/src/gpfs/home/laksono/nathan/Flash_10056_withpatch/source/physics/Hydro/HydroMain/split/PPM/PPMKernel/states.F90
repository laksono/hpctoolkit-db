...
!!! ---------LOCAL -----------------------------
  real, dimension(numCells) :: pallpl, pallml, pall0l
...
!
  ...
  do i = 5, numIntCells5
     ...
     cflno(i)  = max (0.e00, scrch1(i))
     scrch1(i) = 0.5e00 * min (1.e00, cflno(i))
...
!       teristics, respectively.
  ...
  do i = 5, numIntCells5
     hy_clft(i)  = sqrt (gamc(i-1) * pallpl(i) * rhopl(i))
     clft_inv = 1.e0 / hy_clft(i)
     
     scrch1_   = 0.5e00 * (upl(i) - uml(i) - (ppl(i) - pml(i)) &
          * clft_inv)
     
     scrch2_   = (ppl(i) - p0l(i)) * clft_inv * clft_inv &
          + 1.e00/rhopl(i)
     
     scrch2_   = scrch2_ - 1.e00 / rho0l(i)
     ...
     hy_vlft(i)   = 1.e00 / rhopl(i) - scrch2_ - scrch1_ * clft_inv
...
!       teristics, respectively.
  ...
  do i = 5, numIntCells5
     hy_crght(i)  = sqrt (gamc(i) * pallml(i) * rhoml(i))
     crght_inv = 1.e0 / hy_crght(i)
     
     scrch1_   = -0.5e00 * (uml(i) - upl(i) + (pml(i) - ppl(i))  &
          * crght_inv)
     
     scrch2_   = (pml(i) - p0l(i)) * crght_inv * crght_inv  &
          + 1.e00 / rhoml(i)
     ...
     hy_vrght(i)  = 1.e00 / rhoml(i) - scrch2_ - scrch1_ *crght_inv
