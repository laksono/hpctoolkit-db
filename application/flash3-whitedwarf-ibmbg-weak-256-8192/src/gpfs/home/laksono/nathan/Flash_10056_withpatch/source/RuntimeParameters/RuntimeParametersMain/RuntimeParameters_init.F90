...
!! SYNOPSIS
!!
!!  RuntimeParameters_init(integer(in)  :: myPE,
!!                         logical(out) :: restart)
!!
!!
!! DESCRIPTION
...
subroutine RuntimeParameters_init(myPE, restart)
...
#include "Flash_mpi.h"
  ...
  call MPI_Bcast(flags, 2, FLASH_INTEGER, MASTER_PE, MPI_COMM_WORLD, ierr)
  ...
  if (checkpointFlagExists == 1) then
     restart = .true.
     call IO_initRPsFromCheckpoint(myPE, checkpointFile, ierr)
  end if
  ...
  if (MyPE == MASTER_PE) then
     ...
     call RuntimeParameters_bcast (myPE)
