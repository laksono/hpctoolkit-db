...
! set z index extent
        ...
        if(iopt.gt.1) then
          ...
          if(ndim.eq.1) then
            message_size_fcx(i) = (lnxb+1)*lnyb*lnzb
            message_size_fcy(i) = 0
            message_size_fcz(i) = 0
            message_size_ec(i) = 0
          elseif(ndim.eq.2.and.l2p5d.eq.0) then
            message_size_fcx(i) = (lnxb+1)*lnyb*lnzb
            message_size_fcy(i) = lnxb*(lnyb+k2d)*lnzb
            message_size_fcz(i) = 0
            message_size_ec(i) = lnxb*(lnyb+k2d)*(lnzb+k3d) + & 
     &                           (lnxb+1)*lnyb*(lnzb+k3d)  
          else ! (ndim.eq.3.or.l2p5d.eq.1)
             if (ndim.eq.2.AND.l2p5d.eq.1) then
                message_size_fcx(i) = (lnxb+1)*lnyb*lnzb
                message_size_fcy(i) = lnxb*(lnyb+k2d)*lnzb
                message_size_fcz(i) = 0
             else
                message_size_fcx(i) = (lnxb+1)*lnyb*lnzb
                message_size_fcy(i) = lnxb*(lnyb+k2d)*lnzb
                message_size_fcz(i) = lnxb*lnyb*(lnzb+k3d) 
             end if
            message_size_ec(i) = lnxb*(lnyb+k2d)*(lnzb+k3d) + & 
     &                           (lnxb+1)*lnyb*(lnzb+k3d) + & 
     ...
     &  message_size_fcx(i)+message_size_fcy(i)+message_size_fcz(i),message_size_ec(i)
#endif /* DEBUG */
