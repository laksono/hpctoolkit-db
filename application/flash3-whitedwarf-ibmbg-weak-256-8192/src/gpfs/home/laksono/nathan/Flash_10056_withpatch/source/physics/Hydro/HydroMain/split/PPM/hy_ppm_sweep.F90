...
subroutine hy_ppm_sweep ( myPE, numProcs,    &
                         blockCount, blockList, &
                         timeEndAdv, dt, dtOld,  &
                         sweepDir )

  use Hydro_data, ONLY : hy_fluxCorrect,  hy_gravMass,    &
...
#endif 	 
  call Grid_fillGuardCells(myPE, CENTER, ALLDIR, eosMode=hy_eosMode,doEos=.FALSE.,&
       maskSize=hy_gcMaskSize, mask=hy_gcMask, makeMaskConsistent=.true.)


!DEV: We may want to restore this at some point.  DO NOT DELETE!
!!$  call Grid_fillGuardCells(myPE, CENTER, idir,minLayers=hy_transverseStencilWidth)
...
!!------------------------------------------------------------------
  ...
  do blk=1,blockCount
     ...
     allocate(tempFlx(NFLUXES,isizeGC,jsizeGC,ksizeGC),stat=istat)

     allocate(tempArea(isizeGC,jsizeGC,ksizeGC),stat=istat)
     allocate(tempGrav1d_o(isizeGC,jsizeGC,ksizeGC),stat=istat)
     allocate(tempGrav1d(isizeGC,jsizeGC,ksizeGC),stat=istat)
     allocate(tempDtDx(isizeGC,jsizeGC,ksizeGC),stat=istat)
     allocate(tempFict(isizeGC,jsizeGC,ksizeGC),stat=istat)
     allocate(tempAreaLeft(isizeGC,jsizeGC,ksizeGC),STAT=ISTAT)

     allocate(shock(isizeGC,jsizeGC,ksizeGC),stat=istat)

     allocate(primaryCoord(numCells),stat=istat)
     allocate(primaryLeftCoord(numCells),stat=istat)
     allocate(primaryRghtCoord(numCells),stat=istat)
     allocate(primaryDx(numCells),stat=istat)
     allocate(secondCoord(numCells),stat=istat)
     allocate(thirdCoord(numCells),stat=istat)
     allocate(radialCoord(numCells),stat=istat)
     allocate(ugrid(numCells),stat=istat)
...
!!--------- DO HYDRO ON A BLOCK
     ...
     call hy_ppm_block(myPe, blockList(blk),sweepDir, dt, dtOld, &
...
!!--------- FLUX CONSERVATION
     if (hy_fluxCorrect) then
        ...
        call Grid_putFluxData(blockList(blk),sweepDir,tempFlx,size, hy_specialFluxVar, tempAreaLeft)
...
! Do this part only if refining and flux correcting
  ...
  if(hy_fluxCorrect) then
     ...
     call Grid_conserveFluxes(MyPE,sweepDir, level)
     ...
     do blk = 1,blockCount
        ...
        call hy_ppm_getTemporaryData(sweepDir,blockList(blk),size,&
             tempArea,tempDtDx,tempGrav1d,tempGrav1d_o,tempFict, tempAreaLeft)
        call Grid_getFluxData(blockList(blk),sweepDir, tempFlx,size, hy_specialFluxVar, tempAreaLeft)
