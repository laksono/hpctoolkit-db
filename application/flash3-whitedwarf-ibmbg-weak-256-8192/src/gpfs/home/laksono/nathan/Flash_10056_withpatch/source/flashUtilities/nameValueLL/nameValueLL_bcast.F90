...
#include "Flash_mpi.h"  
  ...
  if (n_real > 0) then
     ...
     if (myPE /= MASTER_PE) then
        do i = 1, n_real
           call nameValueLL_add(context, real_names(i), real_vals(i))
        enddo
     endif
  ...
  if (n_int > 0) then
     ...
     if (myPE /= MASTER_PE) then
        do i = 1, n_int
           call nameValueLL_add (context, int_names(i), int_vals(i))
        enddo
     endif
  ...
  if (n_str > 0) then
     ...
     if (myPE /= MASTER_PE) then
        do i = 1, n_str
           call nameValueLL_add (context, str_names(i), str_vals(i))
        enddo
     endif
  ...
  if (n_log > 0) then
     ...
     if (myPE /= MASTER_PE) then
        do i = 1, n_log
           call nameValueLL_add(context, log_names(i), log_vals(i))
        enddo
     endif
