...
!!
!! SYNOPSIS
!!  Hydro_detectShock( real, pointer:: solnData(:,:,:,), 
!!                     real(OUT)    :: shock(:,:,:), 
...
! get a pointer to all the data for the current block
  ...
  ib = blkLimitsGC(LOW,IAXIS)+1
  ie = blkLimitsGC(HIGH,IAXIS)-1
  jb = blkLimitsGC(LOW,JAXIS)+K2D
  je = blkLimitsGC(HIGH,JAXIS)-K2D
  kb = blkLimitsGC(LOW,KAXIS)+K3D
  ke = blkLimitsGC(HIGH,KAXIS)-K3D
  ...
  if((NDIM ==3).and.(hy_geometry/=CARTESIAN).and.(hy_geometry/=CYLINDRICAL))&
       call Driver_abortFlash("SHOCK DETECT : 3d spherical not supported")
#endif
