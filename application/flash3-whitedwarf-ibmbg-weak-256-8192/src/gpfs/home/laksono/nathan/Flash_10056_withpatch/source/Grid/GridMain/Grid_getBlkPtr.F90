...
#include "Flash.h"
  ...
  use Grid_data, ONLY : scratch, gr_mype, gr_blkPtrRefCount, gr_lastBlkPtrGotten, &
  ...
  if (gds .eq. CENTER .or. gds .eq. FACEX .or. gds .eq. FACEY .or. gds .eq. FACEZ) then
     ...
     if (blkPtrRefCount .ne. 0 ) then 
        ...
        call amr_1blk_guardcell(gr_mype,iopt,nlayers,blockId,gr_mype, &
             lcc,lfc,lec,lnc, &
...
#else !  FL_NON_PERMANENT_GUARDCELLS
     case(CENTER)
        dataPtr => unk1(:,:,:,:,idest)
     case(FACEX)
