  ...
  do i = 1,gr_numRefineVarsMax
     ...
     if(paramString /= "none") then
        ...
        if (refVar > 0) then
           ...
           if (MyPE==MASTER_PE) print*, 'WARNING: Unrecognized variable name in refine_var_',i,' treating it as "none"'
           call Logfile_stampMessage(MyPE,'WARNING: Unrecognized variable name in refine_var, treating it as "none"')
