...
!!                          ibnd=-1, the i index applied when filling unk will run
...
!  bcType = gr_domainBC(face,bcDir)
  ...
  print*,mype, ibc, bcDir,ibnd,ibegin,iend
...
contains
  ...
  subroutine internal_getBlkHandle(blockID, pe, accessBlkOut)
    ...
    subroutine prioselector(bcTypeToApply,directionToApply,leftOrRight,bcTypeFromParamesh)
      ...
      integer :: celery(2,2,2),celery0(2,2,2) !for faster(?) access to relevant surrblks info - KW
