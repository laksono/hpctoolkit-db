...
!! SYNOPSIS
!!
!!  call hy_ppm_block(integer(IN):: myPE,
!!                integer(IN):: sweepDir, 
!!                integer(IN):: blockID,
!!                real(IN)   :: dt, 
!!                real(IN)   :: dtOld, 
...
subroutine hy_ppm_block( myPe,blockID,sweepDir, dt, dtOld, &
...
#include "PPM.h"
  ...
  !! ------------
  !! ---- ARGUMENTS
  integer, intent(IN) :: myPe, blockID
  ...
  if (istat==0)allocate( hy_shockd(numCells),stat = istat) !DEV: set in flaten, but never referenced anywhere - KW
