...
!!
!!  SYNOPSIS
!!     call gr_expandDomain(integer (IN)  :: myPe,
!!                          integer (IN)  :: numProcs,
!!                          logical,(OUT) :: particlesInitialized)
!!
!!  DESCRIPTION
!!
...
subroutine gr_expandDomain (myPe,numProcs,particlesInitialized)


#include "Flash.h"
  use Grid_data, ONLY : gr_domainBC,gr_eosModeInit,gr_refineOnParticleCount,&
       gr_refineOnPdens,gr_maxParticlesPerBlk,gr_minParticlesPerBlk, gr_myPE
  ...
  use tree, ONLY : lrefine, lrefine_max, grid_changed
  ...
  use Particles_interface, ONLY : Particles_accumCount, &
    Particles_initPositions, Particles_getCountPerBlk, &
...
!!        particlesPosnsDone=particlesPosnsDone.and.updateRefine
        ...
        call Particles_initPositions(myPE,particlesPosnsDone,updateRefine)
     end if
...
#endif           
        if(gr_refineOnParticleCount.and.upDateRefine)&
