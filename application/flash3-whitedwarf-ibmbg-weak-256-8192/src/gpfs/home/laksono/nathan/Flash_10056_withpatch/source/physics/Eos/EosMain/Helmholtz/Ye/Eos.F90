...
!!             For the Ye formulation, this value is never used!
!!
...
subroutine Eos(mode,vecLen,eosData,massFrac,mask)
  ...
  use Eos_data, ONLY: eos_smallT, eos_tol, eos_maxNewton, &
       eos_forceConstantInput, eos_myPE
...
#include "Eos.h"
  ...
  !  necessarily have to have mass fractions.  
   ...
   ! warn user if mask is used. Helmholtz EOS doesn't use it very much
  maskInternal = .false.
  if (present(mask)) then
     ...
     if (firstCall) then
        ...
        if (eos_myPE .EQ. MASTER_PE) write(*,*)"[EOS Helmholtz] WARNING!  Mask setting does not speed up Eos Helmholtz calls"
        firstCall = .false.
     end if
  end if
  ...
  do k = 1, vecLen
     ...
     zbarRow(k)    = eosData(zbar+k)   ! NOTE compare to normal Helmholtz where we don't assume
...
70      continue           
     ...
     do k = vecBegin, vecEnd
        ...
        call Driver_abortFlash('[Eos] Error: too many iteration in Newton-Raphson')
