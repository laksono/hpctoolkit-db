...
subroutine Hydro ( myPE, numProcs,        &
                   blockCount, blockList, &
                   timeEndAdv, dt, dtOld, &
                   sweepOrder )
  use hy_ppm_interface, ONLY : hy_ppm_sweep
  use Hydro_data, ONLY:  hy_gravMass, hy_gravMassXYZ, hy_gravMassZYX,&
...
#include "Flash.h"
  ...
  case (SWEEP_XYZ)
     ...
     call hy_ppm_sweep ( myPE, numProcs,        &
                        blockCount, blockList,  &
                        timeEndAdv, dt, dtOld,  &
                        SWEEP_X )
     ...
     if (NDIM >= 2) call  hy_ppm_sweep( myPE, numProcs,       &
                                       blockCount, blockList, &
                                       timeEndAdv, dt, dtOld, &
                                       SWEEP_Y )
     
     if (NDIM == 3) call  hy_ppm_sweep( myPE, numProcs,       &
                                       blockCount, blockList, &
                                       timeEndAdv, dt, dtOld, &
                                       SWEEP_Z )
