...
!!    gridDataStruct - In PM3 it can have values(CENTER,FACEX,FACEY,FACEZ,WORK),
...
!!    regionType     - The part of the block that is involved in boundary condition. this integer
...
!!  real,allocatable,dimension(:) :: cellCenterSweepCoord, secondCoord,thirdCoord
  ...
  call Grid_bcApplyToRegionSpecialized(bcType,gridDataStruct,&
       ...
       blockHandle,nextDir(1),nextDir(2),endPoints,blkLimitsGC)
  if(.not.applied) then
