...
!!***
  ...
  subroutine intrfc(numIntCells, numCells, guard, &
       &            rho, u, ut, utt, p, &
       &            rhol, rhor, ul, ur, &
       &            utl, utr, uttl, uttr, &
       &            pl, pr, vl, vr, gamcl, &
       &            gamcr, game, &
       &            gamel,gamer,gamc,grav, xn, &
       &            xnl, xnr, v, dx, x)

  use Hydro_data,   ONLY:  hy_numXn, hy_smlrho, hy_smallx, hy_gravl, &
                           hy_drho, hy_rho6, &
...
#include "Flash.h"
  ...
  real, intent(IN), DIMENSION(numCells) :: &
       rho, u, ut, utt, p, gamc, game, grav, dx, x
...
! each zone
  ...
  call interp(numIntCells,numCells,rhol, rho, rhor, &
              coeff1, coeff2, coeff3, coeff4, coeff5)


! search for contact discontinuities, and steepen the profile if we find them
! -- this prevents the contant from spreading out over too many zones as it
! propagates
...
! interpolate the remainder of the variables 
  ...
  call interp(numIntCells, numCells,uttl,utt,uttr,&
              coeff1,coeff2,coeff3,coeff4,coeff5)
  call interp(numIntCells, numCells,pl,p,pr,coeff1,coeff2,coeff3,coeff4,coeff5)
  call interp(numIntCells, numCells,gamel,game,gamer,&
              coeff1,coeff2,coeff3,coeff4,coeff5)
  call interp(numIntCells, numCells,gamcl, gamc, gamcr,&
              coeff1,coeff2,coeff3,coeff4,coeff5)
  
  call interp(numIntCells, numCells,hy_gravl, grav, hy_gravr,  &
              coeff1,coeff2,coeff3,coeff4,coeff5)
