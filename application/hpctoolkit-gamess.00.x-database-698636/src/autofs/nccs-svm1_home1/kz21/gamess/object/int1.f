C 29 Jan 17 - ADF - 'thread-safe' version of STVINT (STVINTTS)          
C 18 Apr 16 - HN  - CHANGES FOR FMO 5.2                                 
C 22 Sep 15 - HL  - CHANGES FOR QUANPOL                                 
...
C>                                                                      
C>    @brief    driver for conventional h, S, and T integrals           
C>                                                                      
C>    @details  compute one electron integrals,                         
C>                S is evaluated by Gauss-Hermite quadrature,           
C>                T is an overlap with -2,0,+2 angular momentum shifts, 
C>                V is evaluated by Gauss-Rys quadrature, then h=T+V    
C>              Also, do Lz integrals for atoms and linear cases.       
C>              If one is interested, the routine can do the x,y,z      
C>              components of kinetic energy separately.                
C>              If one is interested, the routine can do the Lx and     
C>              Ly components of angular momentum too.                  
C>                                                                      
C>    @author   The original was called STANDV in Michel Dupuis' HONDO. 
C>              Many have modified it since then.                       
C>                                                                      
      SUBROUTINE HSANDT(H,S,T,Z,ESP1E,NLCT,ATCHRG,LL2,SOME,LZINT,DOESP0,
     *                  DOESP1,DCCUT,DBUG)                              
      USE constants, ONLY: W1 => sqrt_PI                                
      use mx_limits, only: mxsh,mxgtot,mxatm,maxden,mxabc,maxsh         
C                                                                       
C$    USE omp_int1, ONLY: omp_hst                                       
...
C                                                                       
C$    IF ((intomp.NE.0).AND.(runtyp.NE.MOROKM)) THEN                    
C$      CALL omp_hst(h,s,t,z,esp1e,nlct,atchrg,ll2,nat1e,lfmoc,         
C$   *              lzint,doesp0,                                       
C$   *              doesp1,dccut,dbug)                                  
C$      RETURN                                                          
C$    END IF                                                            
      UNCON=RMETHOD.NE.RNONE.AND.MOD(MODQR,2).EQ.1                      
      IF(RMETHOD.EQ.RLUT) UNCON = .FALSE.                               
C                                                                       
      TOL = RLN10*ITOL                                                  
...
C                                                                       
      ...
      nqmmatm = 0                                                       
      if(nfg.ne.0) then                                                 
        call setatz(1,NATED,fastvesp,lfvesp,needfv)                     
c       write(6,*) "wwwint1 =",nchmat,nated                             
        if(QMMM) call getmmchg(nqmmatm,dum)                             
      end if                                                            
...
C                                                                       
                   IF(.NOT.DCFSKP) THEN                                 
C                                                                       
                     IF(RMETHOD.EQ.RLUT.AND.IAT.EQ.JAT) GOTO 330        
C                                                                       
...
C                                                                       
                  DO 460 IC = NATST,NATED+nqmmatm                       
                                                                        
C                                                                       
                     IF(RMETHOD.EQ.RLUT.AND.IAT.EQ.JAT.AND.IAT.EQ.IC)   
     *                 GOTO 460                                         
C                                                                       
                     IF(IC.LE.NAT) THEN                                 
                        ZNUC = -ZAN(IC)                                 
...
c                        FRACESP=x(icind+1)                             
                         FRACV   = one                                  
                         FRACESP = zero                                 
                         icind   = lfvesp+nated*6                       
                         icind   = icind +(ic-nated-1)*4                
                         CX      = x(icind  )                           
                         CY      = x(icind+1)                           
                         CZ      = x(icind+2)                           
                         ZNUC    =-x(icind+3)                           
...
c    *                            cx,cy,cz                              
                     ...
                     END IF                                             
c                  WRITE(6,1234) 'WWWATO',IC,ii,jj,ZNUC,fracesp,cx,cy,cz
c1234                format(A6,3I4,5F10.5)                              
C                                                                       
...
C-LXYZ-  CALL DDI_GSUMF(926,LZ,L2)                                      
      ...
      END IF                                                            
c                                                                       
c        WRITE(IW,*) 'V INTEGRALS'                                      
c        CALL PRTRIL(H,L1)                                              
c     IF(DOESP1) then                                                   
c        WRITE(IW,*) 'ESP1 INTEGRALS'                                   
c        CALL PRTRIL(ESP1E,L1)                                          
c        call abrt                                                      
c     endif                                                             
C                                                                       
...
C                                                                       
      ...
      IF(RMETHOD.EQ.RLUT) CALL DAWRIT(IDAF,IODA,H,LL2,396,0)            
C                                                                       
...
C>                                                                      
C>    @brief    driver for all 1e- integrals                            
C>                                                                      
C>    @details  compute one electron integrals, h=V(ne) + T, S, T       
C>                   with various modifications to "h" such as:         
C>              small component elimination schemes, namely             
C>                   RESC, DK, IOTC, LUT-IOTC, or NESC,                 
C>              Effective or Model Core Potential integrals,            
C>              Effective Fragment Potential integrals,                 
C>              QUANPOL integrals,                                      
C>              Fragment Molecular Orbital method integrals.            
C>                                                                      
C>    @author   frequently modified (a cast of thousands)               
C>                                                                      
...
C>       electronic contributions                                       
      ...
      SUBROUTINE ONEEI                                                  
C                                                                       
      USE MX_LIMITS,ONLY:MXFRG,MXDFG,MXDPPT,mxsh,mxgtot,mxatm,mxao      
      USE comm_FRGINF                                                   
      USE comm_EFPFMO                                                   
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
     ...
     *        DOPCM,LCUT,DOVLMO,DOEFP,DOREP,outpcm                      
      LOGICAL DFTBFL,SCC,SRSCC,DFTB3,DAMPXH,LRDFTB                      
      LOGICAL DOECP1,DOECP2                                             
C                                                                       
      CHARACTER*2 WORDS(3)                                              
      dimension idamdt(3)                                               
C                                                                       
...
C                                                                       
     ...
     *            X(LATCHRG),L2,SOME,LZINT,DOESP0,DOESP1,DCCUT,DBUG)    
      IF(QUADRE) THEN                                                   
         IF(RMETHOD.EQ.RLUT) THEN                                       
            CALL QSANDTLUT(X(LS),X(LT),X(LQQ),X(LQQ+L2*NDQ),L2,SOME)    
         ELSE                                                           
            CALL QSANDT(X(LS),X(LT),X(LQQ),X(LQQ+L2*NDQ),L2,SOME)       
         END IF                                                         
      END IF                                                            
c$$$      IF(QUADRE) CALL QSANDT(X(LS),X(LT),X(LQQ),X(LQQ+L2*NDQ),L2,SOM
      IF(DOVLMO) CALL ADDLMOCH( ONE,X(LNATFRG),X(LNFGLMO),X(LLFGLMO),   
     *                         X(LPFGLMO),X(LPOPMAT))                   
...
C                                                                       
      IF(RMETHOD.EQ.RLUT) THEN                                          
        IF(SOME) WRITE(IW,9600)                                         
          IF(MOD(MODQR/2,2).NE.0.AND.SOME) WRITE(IW,9210)               
          CALL FLIPBASIS(17)                                            
          CALL LUTX(X(LH),L1,L2,DBUG)                                   
          IF(ISAVE.EQ.1) CALL DAWRIT(IDAF,IODA,X(LH),L2,311,0)          
          CALL FLIPBASIS(15)                                            
      END IF                                                            
C                                                                       
      IF(UNCON) THEN                                                    
...
C        KEEP REPULSION INTEGRALS INTACT IN X(LH)                       
         CALL XAXPY(L2,-ONE,X(LT),1,X(LH),1)                            
C                                                                       
...
C     THE ORDER OF MEMORY IS IMPORTANT HERE!!!                          
      DOECP1 = IECP.GT.0.AND.IECP.NE.5.AND.NFG.EQ.0                     
C                                                                       
      IF(DOECP1.or.DOECP2) THEN                                         
         LHECP = LWRK                                                   
         CALL VALFM(LOADFM)                                             
         LDCF1  = LOADFM + 1                                            
         LJLN   = LDCF1  +  NCOEF1                                      
...
C                                                                       
     ...
     *                  X(LFF2SIG2),X(LFF2EPS2),X(LFFCORDMN),           
     ...
     *                  X(LFFUMBHIS),X(LFFUM2HIS),X(LFFMERGE),          
     *                  X(LFFINTLST),X(LFFCHGIMG),X(LFFCRDIMG),         
...
C     Coulomb electronic contributions                                  
      LVLEFP=LEVELEFP()                                                 
      IF(LVLEFP.EQ.2) THEN                                              
       IF(IEFC.EQ.1) CALL EFCINT_EFP2(X(LWRK),X(LH))                    
       IF(IEFD.EQ.1.OR.DOESP2.AND.IVMUL.GT.1)                           
     *               CALL EFDINT_EFP2(X(LWRK),X(LH),                    
     *               X(LUNTXYZ),X(LVMUL),LDAV,X(LINDVMUL),X(LFMOESPB))  
       IF(IEFQ.EQ.1.OR.DOESP2.AND.IVMUL.GT.2)                           
     *               CALL EFQINT_EFP2(X(LWRK),X(LH),                    
     *               X(LUNTXYZ),X(LVMUL),LDAV,X(LINDVMUL),X(LFMOESPB))  
       IF(IEFO.EQ.1.OR.DOESP2.AND.IVMUL.GT.3)                           
     *               CALL EFOINT_EFP2(X(LWRK),X(LH),                    
     *               X(LUNTXYZ),X(LVMUL),LDAV,X(LINDVMUL),X(LFMOESPB))  
      ELSE                                                              
...
c                                                                       
     ...
     *                  X(LH),X(LPOTCUR),X(LPOTOLD),X(LS))              
      ...
      IF(ADDESP) THEN                                                   
        CALL DAREAD(IDAF,IODA,X(LH),L2,11,0)                            
        CALL XAXPY(L2,ONE,X(LFMOESPA),1,X(LH),1)                        
...
C                                                                       
 ...
 9500 FORMAT(/1X,73(1H*)/                                               
     ...
     *        ' 130, 164114 (2009).'//                                  
     *        1X,73(1H*)/)                                              
 9600 FORMAT(/1X,70(1H=)/                                               
     * 1X,5X,'INFINITE-ORDER TWO-COMPONENT (IOTC) ONE-',                
     *    'ELECTRON HAMILTONIAN'/                                       
     * 1X,5X,'WITH LOCAL UNITARY TRANSFORMATION (LUT) SCHEME'/          
     * 1X,5X,38X,'CODED BY YUYA NAKAJIMA'/                              
     * 1X,5X,7X,'J. SEINO, H. NAKAI, J. CHEM. PHYS. 136, 244102 (2012)'/
     * 1X,5X,7X,'J. SEINO, H. NAKAI, J. CHEM. PHYS. 137, 144101 (2012)'/
     * 1X,5X,7X,'J. SEINO, H. NAKAI, J. CHEM. PHYS. 139, 034109 (2013)'/
     * 1X,5X,2X,'J. SEINO, H. NAKAI, INT. J. QUANTUM. CHEM. 115, ',     
     *       '253 (2015)'/                                              
     * 1X,70(1H=)/)                                                     
      END                                                               
...
C*MODULE INT1    *DECK STVINT                                           
C>                                                                      
C> @brief Gauss-Hermite quadrature (integrals)                          
C>                                                                      
C> @details partially 'thread-safe' version of STVINT.  No DSTV.        
C>                                                                      
C> @date August 2016 - Alex Findlater                                   
C>      -a copy of STVINT except the STV common block                   
C>       has been removed and vars are passed directly.                 
C>       STVINT still exists.                                           
C>                                                                      
      SUBROUTINE STVINTTS(XINT,YINT,ZINT,T,                             
     *                  X0,Y0,Z0,XI,YI,ZI,XJ,YJ,ZJ,NI,NJ)               
      USE gausshermite, ONLY: H, W                                      
C                                                                       
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)                               
C                                                                       
...
C$omp threadprivate(/ROOT/)
     ...
     *                IQRORD,MODQR,NESOC,NRATOM,                        
     *                NUMU,NQMTR,NQRDAF,MORDA,NDARELB                   
c$$$      COMMON /RELWFN/ RMETHOD,QRQMT,CLIG,CLIG2,QRTOL,IQRORD,MODQR,NE
c$$$     *                NRATOM,NUMU,NQMTR,NQRDAF,MORDA,NDARELB        
      COMMON /WFNOPT/ SCFTYP,VBTYP,DFTYPE,TDDFTYP,CITYP,CCTYP,          
     *                MPLEVL,MPCTYP                                     
C                                                                       
...
C                                                                       
         CALL XAXPY(LL2,FSC2/TWO,H,1,S,1)                               
         CALL DAREAD(IDAF,IODA,H,LL2,13,0)                              
         CALL XAXPY(LL2,ONE,H,1,S,1)                                    
...
c     DOESP=NFG.NE.0.AND.ICURFG.NE.0.and.                               
c    *      (iand(ixesp,8).eq.0.or.ifmostp.ne.2)                        
      DOHOP=NFG.NE.0.AND.NBDFG.NE.0.AND.IFMOSTP.NE.6                    
      DOHOP2=NFG.NE.0.AND.IFMOSTP.NE.6.AND.IFMOSTP.GT.1                 
C     For FMO ECP                                                       
      DOECP =NFG.NE.0.AND.IFMOSTP.NE.6.AND.IECP.GT.0.AND.IECP.NE.5      
C                                                                       
      ADDESP=.FALSE.                                                    
