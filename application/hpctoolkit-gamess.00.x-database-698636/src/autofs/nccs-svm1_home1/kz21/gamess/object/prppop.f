...
C 19 DEC 12 - JI  - Add h/i functionality to PROPTY,SHLPOP              
C 19 Oct 12 - MWS - synchronize FRGINF,EFMO commons                     
C  2 Sep 12 - MWS - synchronize MCINP                                   
C 21 Aug 12 - MWS - always execute serially through NBO interface       
...
C 15 APR 11 - MK  - MULKEN: UNDO NOV 2008 CHANGE                        
C  1 OCT 10 - CS  - ADDED CHANGES FOR EFMO                              
C 11 AUG 10 - DGF - ADD LOCAL HARDNESS                                  
...
C                                                                       
      DO 390 I=IAT1,IAT2                                                
         ...
         DO 330 J=IP1,NAT                                               
            ...
            B=ZERO                                                      
            DO 320 K=IB,IE                                              
                         B = B + XDOT(JE-JB+1,QT(K,JB),L1,QT(JB,K),1)   
               IF (OPEN) B = B + XDOT(JE-JB+1,QS(K,JB),L1,QS(JB,K),1)   
  320       CONTINUE                                                    
            BORD(I,J) = B                                               
            BORD(J,I) = B                                               
            VB(I) = VB(I) + B                                           
...
C                                                                       
  ...
  350    CONTINUE                                                       
         ...
         X = ZERO                                                       
         DO 370 MU=IB,IE                                                
            X = X + XDOT(IE-IB+1,QT(MU,IB),L1,QT(IB,MU),1)              
  370    CONTINUE                                                       
         VT(I) = QE - X                                                 
  390 CONTINUE                                                          
C                                                                       
...
C>                                                                      
C> @brief   Driver routine for property analyses                        
C>                                                                      
C> @details perform property calculation for density matrix found       
C>          in direct access file.  Call this as soon as possible       
C>          after a density is written, in case a higher level          
C>          density is computed later in this run.                      
C>                                                                      
C> @author  Mike Schmidt                                                
C>                                                                      
C> @date    December 21, 2012 - Joe Ivanic                              
C>          Allow for population analyses if h,i functions present      
C>                                                                      
C> @date    July, 2019        - Christian Friedl                        
C>          Added PRPTYP=TDD2 which is used by TDGRAD if ITDPRP=2       
C>                                                                      
      SUBROUTINE PROPTY(PRPTYP)                                         
C                                                                       
      USE MX_LIMITS,ONLY:MXFRG,MXDFG,MXDPPT,mxatm,mxao,mxrt,mxnoro      
      USE comm_FRGINF                                                   
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
...
C                                                                       
     ...
     *        CANONC,FCORE,FORS,EKT,LINSER,DETDEN,                      
     *        ISEPS,USEPS,MNMEDG,MNMEOP,UNVGSS,DGAPRX,RDCISV,           
     *        TRIPLET,SG1T,DCFLG,CUM,LSHORT,TAMMD,TPA,ALPHKWD,BETAKWD   
C                                                                       
C                                                                       
...
C                                                                       
      IF(MASWRK) THEN                                                   
         ...
         IF(PRPTYP.EQ.'CISE') WRITE(IW,9270) CITYP,ISTATE               
         IF(PRPTYP.EQ.'CISR') WRITE(IW,9280) CITYP,ISTATE               
         ...
         IF(PRPTYP.EQ.'TDD2') WRITE(IW,9330) DFTYPE,NTHST               
      END IF                                                            
C                                                                       
...
C                                                                       
      ...
      IF(IPCM.EQ.1.OR.IEF.EQ.1) NSOLV=NSOLV+1                           
...
C                                                                       
         ...
         IF(NSTATAVG.GT.1) THEN                                         
            IF(IDEGAVG.EQ.1) THEN                                       
               IF(MASWRK) WRITE(IW,9030)                                
            ELSE                                                        
               IF(MASWRK) WRITE(IW,9040) NSTATAVG                       
               RETURN                                                   
C              ******                                                   
            END IF                                                      
         END IF                                                         
C                                                                       
C     LIE ABOUT NUMBER OF MCSCF E-, THIS MUST BE UNDONE BEFORE EXIT     
C                                                                       
         IF(CISTEP.EQ.ALDET  .OR.                                       
     *      CISTEP.EQ.GENCI  .OR.                                       
     *      CISTEP.EQ.ORMAS  .OR.                                       
     *      CISTEP.EQ.GMCCI) NOCC = NCORSV + NACT                       
         IF(CISTEP.EQ.GUGA ) NOCC = NMCC+(NDOC+NALP+NAOS+NBOS+NVAL)     
         NASAVE = NA                                                    
         NA = NOCC                                                      
      END IF                                                            
C                                                                       
...
C                                                                       
      ...
      LVLEFP = LEVELEFP()                                               
      IF(CCTYP.NE.RNONE.AND.CCTYP.NE.EOMCCSD.AND.LVLEFP.NE.2) GO TO 101 
C AAD I HAVE FIXED ENGANL TO PROPERLY ACCOUNT FOR ALL ENERGIES IN EFP   
      IF (NSOLV.GT.0 .AND. NFRG.EQ.0) GO TO 101                         
      IF(NSOLV.GT.1) GO TO 101                                          
      IF(PRPTYP.EQ.'TDD2') GO TO 101                                    
C                                                                       
      CALL VALFM(LOADFM)                                                
      LDA    = LOADFM + 1                                               
      LDB    = LDA    + L2                                              
...
C                                                                       
      ...
      IF(CCTYP.NE.RNONE.AND.CCTYP.NE.EOMCCSD.AND.LVLEFP.EQ.2) GO TO 100 
C                                                                       
...
C                                                                       
      IF(IEMOM.NE.0) THEN                                               
         CALL ELMOMC                                                    
         IF(NFRG.GT.0) CALL EFDIPOLE                                    
         IF(NFRG.GT.0.OR.IPCM.EQ.1) CALL ELMOMC_SOLV                    
      ENDIF                                                             
...
C                                                                       
C        ----- (SOUMEN SAHA AND RAM K. ROY)'S HARDNESS ANALYSIS -----   
C                                                                       
      IF(IAHARD.GE.1.AND.IAHARD.LE.3) CALL HARDAN                       
C                                                                       
...
C                                                                       
      ...
      CALL TIMIT(1)                                                     
C                                                                       
C     INTERFACE TO THE NATURAL BOND ORBITAL (NBO) ANALYSIS PROGRAM      
C     OF FRANK WEINHOLD AND ERIC GLENDENING.  FOR MORE INFO, SEE        
C            HTTP://WWW.CHEM.WISC.EDU/~NBO6                             
C     NBO IS PROVIDED THE DENSITY TYPE, TO LET IT DECIDE WHAT IT        
C     CAN OR CAN'T ANALYZE.                                             
C     NOTE THAT NBO IS STRICTLY A PROGRAM FOR SEQUENTIAL EXECUTION,     
C     SO ANY OTHER PROCESSES SHOULD SUSPEND HERE UNTIL NBO COMPLETES.   
C                                                                       
      IF(MASWRK) THEN                                                   
         CALL RUNNBO(PRPTYP)                                            
         CALL DDI_SYNC(5301)                                            
...
C                                                                       
 ...
 9050 FORMAT(/1X,'   **** A SOLVENT MODEL IS IN USE IN THIS RUN ****'/  
     ...
     *      ' SOLUTE.'/                                                 
     *   1X,'THE ONE EXCEPTION IS THE DIPOLE, WHICH IS ALSO PRINTED',   
...
C                                                                       
 ...
 9260 FORMAT(/5X,56(1H-)/                                               
     ...
     *        5X,56(1H-))                                               
 9270 FORMAT(/5X,61(1H-)/                                               
     *        5X,A5,' PROPERTIES...FOR THE WAVEFUNCTION OF',            
     *          ' EXCITED STATE',I5/                                    
     *       17X,'USING THE EXPECTATION VALUE DENSITY'/                 
     *        5X,61(1H-))                                               
 9280 FORMAT(/5X,61(1H-)/                                               
     *        5X,A5,' PROPERTIES...FOR THE WAVEFUNCTION OF',            
     *           ' EXCITED STATE',I5/                                   
 ...
 9310 FORMAT(/5X,42(1H-)/                                               
     *        5X,'PROPERTIES FOR THE ',A8,' DFT FUNCTIONAL',            
     *       /5X,' USING THE RELAXED DENSITY OF EXCITED STATE',I5/      
     *        5X,42(1H-))                                               
 ...
 9330 FORMAT(/5X,42(1H-)/                                               
     *        5X,'PROPERTIES FOR THE ',A8,' DFT FUNCTIONAL'/            
     *        5X,' USING THE TRANSITION DENSITY BETWEEN'/               
     *        5X,' THE GROUND STATE AND EXCITED STATE',I5/              
     *        5X,42(1H-))                                               
      END                                                               
...
C>                                                                      
C> @brief   Property calculation                                        
C>                                                                      
C> @details Calculate AO Mulliken population                            
C>                                                                      
C> @author  Unknown                                                     
C>                                                                      
C> @date    February, 2016 - Yoshio Nishimoto                           
C>          - Bug fix for DFTB with d-orbitals                          
C>                                                                      
...
C                                                                       
  ...
  200    CONTINUE                                                       
      ...
      IF (DFTBFL) THEN                                                  
        IF (IPASS.EQ.1) THEN                                            
          CALL DAREAD(IDAF,IODA,OCCNO,L1,562,0)                         
        ELSE IF (IPASS.EQ.2) THEN                                       
          CALL DAREAD(IDAF,IODA,OCCNO,L1,563,0)                         
        END IF                                                          
      END IF                                                            
C                                                                       
...
C                                                                       
      IF(PARALL3) THEN                                                  
         CALL XCOPY((IORB1-1)   *L1,ZERO,0,V(1,1)      ,1)              
         CALL XCOPY((NORB-IORB2)*L1,ZERO,0,V(1,IORB2+1),1)              
      END IF                                                            
C                                                                       
      IF(ISPHER.GE.0.AND..NOT.DFTBFL) THEN                              
         DO J=IORB1,IORB2                                               
            CALL XGEMV('N',L1,L1,ONE,SPI,L1,V(1,J),1,ZERO,T,1)          
            CALL XCOPY(L1,T,1,V(1,J),1)                                 
         ENDDO                                                          
      END IF                                                            
C                                                                       
...
C                                                                       
 ...
 9010 FORMAT(/5X,'ATOMIC MULLIKEN POPULATION IN EACH MOLECULAR ORBITAL')
...
C                                                                       
      ...
      IF(LAST .AND. MASWRK) THEN                                        
         ...
         WRITE(IP,9140)                                                 
      END IF                                                            
      IF(IAHARD.EQ.1) CALL DAWRIT(IDAF,IODA,X(IGAC),NAT,493,0)          
      IF(IAHARD.EQ.2) CALL DAWRIT(IDAF,IODA,X(ILAC),NAT,493,0)          
      DO 210 I=1,NAT                                                    
         PM = X(I-1+IGAC)                                               
         zani=zan(i)                                                    
         if(dftbfl) call dftb_nucz(zani,1)                              
         QM = ZANI - PM                                                 
         PL = X(I-1+ILAC)                                               
         QL = ZANI - PL                                                 
...
C                                                                       
      ...
      CALL XCOPY(NAT,X(I81),1,X(I82),1)                                 
...
C                                                                       
C> @brief : Compute Mulliken spherical harmonic populations             
C>                                                                      
C> @date  : December 21, 2012 - Joe Ivanic                              
C>          Add shell partition for h,i functions                       
C>          Previously there was no capability for h,i - there is now   
C                                                                       
...
C                    CARTESIAN H SHELL HAS P,F CONTAMINANTS             
               ELSE IF(LIT.EQ.6) THEN                                   
                  SHL(2)=SHL(2)+AOPOP(47+LOC)                           
     *                         +AOPOP(48+LOC)+AOPOP(49+LOC)             
                  SHL(4)=SHL(4)+AOPOP(50+LOC)+AOPOP(51+LOC)             
     *                         +AOPOP(52+LOC)+AOPOP(53+LOC)             
     *                         +AOPOP(54+LOC)+AOPOP(55+LOC)             
     *                         +AOPOP(56+LOC)                           
                  MAX=MAX-10                                            
C                                                                       
C                    CARTESIAN I SHELL HAS S,D,G CONTAMINANTS           
               ELSE IF(LIT.EQ.7) THEN                                   
                  SHL(1)=SHL(1)+AOPOP(70+LOC)                           
                  SHL(3)=SHL(3)+AOPOP(71+LOC)+AOPOP(72+LOC)             
     *                         +AOPOP(73+LOC)+AOPOP(74+LOC)             
     *                         +AOPOP(75+LOC)                           
                  SHL(5)=SHL(5)+AOPOP(76+LOC)+AOPOP(77+LOC)             
     *                         +AOPOP(78+LOC)+AOPOP(79+LOC)             
     *                         +AOPOP(80+LOC)+AOPOP(81+LOC)             
     *                         +AOPOP(82+LOC)+AOPOP(83+LOC)             
     *                         +AOPOP(84+LOC)                           
                  MAX=MAX-15                                            
               END IF                                                   
...
C                                                                       
     ...
     *     ROHF/8HROHF    /, RNONE/8HNONE    /, SFCIS/8HSFCIS   /       
      DATA CHECK/8HCHECK   /                                            
C                                                                       
...
C           BUT DOES HAVE THIS FOR SPIN-FLIP CIS.                       
...
C                                                                       
      DO 180 J = J1,J2                                                  
         ...
         DO 140 I = 1,N                                                 
            ...
            VDUM = V(I,J)                                               
            IF (IM1 .GT. 0) THEN                                        
               CALL XAXPY(IM1,VDUM,SH(IK+1),1,T,1)                      
               DUM = XDOT(IM1,SH(IK+1),1,V(1,J),1)                      
            END IF                                                      
            IK = IK+I                                                   
            T(I) = DUM+SH(IK)*VDUM                                      
  140    CONTINUE                                                       
         CALL XCOPY(N,T,1,VO(1,J),1)                                    
  180 CONTINUE                                                          
C                                                                       
...
C>                                                                      
C>    @brief    distributed multipole analysis, ala Stone               
C>                                                                      
C>    @details  computes integrals up to octupoles at the complete      
C>              set of distributed points, generating the multiple      
C>              tensors at all such sites.  This can be run by          
C>              input keywords, but is normally part of MAKEFUP jobs.   
C>                                                                      
C>    @author   Jan Jensen?  Must have existed since EFP1-model began.  
C>                                                                      
C>    @date     January, 2017 - C.Bertoni                               
C>              - Changes to store correct mass for EFMO                
C>                                                                      
...
C                                                                       
      USE MX_LIMITS,ONLY:MXFGPT,mxsh,mxgtot,mxatm,mxao                  
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
...
C     FIRST SHELL LOOP.                                                 
C                                                                       
CCC terribly long loop, very easy to lose track and make a mistake,     
CCC which might be very diffiult to catch from printout - if small      
      DO 9000 II=1,NSHELL                                               
         I=KATOM(II)                                                    
         XI=C(1,I)                                                      
         YI=C(2,I)                                                      
...
C                                                                       
C  LVS 2005                                                             
C  SWITCH BETWEEN ORIGINAL AND MODIFIED STONE PROCEDURES                
...
C                                                                       
CCC can try parallelize with Fortran-specific reduction by minimum      
                  DIST=1.0D+22                                          
                  DO NPT=1,NPTSTN                                       
...
C                                                                       
CCC very tricky to parallelize                                          
                  IEQ=0                                                 
                  DO 67 NPT=1,NPTSTN                                    
                     TEST=(AX-STNPNT(2,NPT))**2                         
     *                   +(AY-STNPNT(3,NPT))**2                         
...
C              IF EFMO, NO PRINTOUT / PUNCHOUT                          
c     IF(IEFMORUN.GT.0) GOTO 9960                                       
C                                                                       
...
C     *                         XX(I),YY(I),ZZ(I),XY(I),XZ(I),YZ(I)     
 ...
 9256 FORMAT(A8,4F16.10,' >'/8X,2F16.10)                                
C                                     OCTUPOLES                         
      WRITE(IW,9260)                                                    
      IF(IDMP.GT.0) THEN                                                
         WRITE(NFTEFP,9261)                                             
...
C     *                       XXZ(I),XYY(I),YYZ(I),XZZ(I),YZZ(I),XYZ(I) 
 ...
 9276 FORMAT(A8,4F17.9,' >'/8X,4F17.9,' >'/8X,2F17.9)                   
C                                                                       
      IF(IDMP.GT.0) THEN                                                
         CALL FLSHBF(NFTEFP)                                            
         CALL FLSHBF(IDMP)                                              
...
!$omp end parallel                                                      
      END IF                                                            
c9960 CONTINUE                                                          
C     BAIL OUT IF NOT CORRECT EFMO RUNTYPE                              
      IF(IEFMORUN.EQ.0.OR.(IEFMORUN.GT.0.AND.IEFMORT.NE.1)) RETURN      
C                                                                       
C NAME, COORDINATES, MASS AND NUCLEAR CHARGE                            
      IF( MASWRK ) THEN                                                 
C      EFMOFRGINFO(IEFMOCFRG) = NPTSTN                                  
C     NB! FMCOM COMMON BLOCK, XXXX IS THE DYNAMIC MEMORY ARRAY,         
      CALL EFMOPTNSAV(XXXX(LNEFMOPTS),IEFMONFRG,IEFMOCFRG,1,NPTSTN)     
C     GET correct masses                                                
c      call fmorams(NPTSTN,xxxx(lfmozan),xxxx(lfmomas),ZMSSTN,2)        
c$$$  making sure the indexing into fmomas is right                     
      call fmorams(NPTSTN,xxxx(lfmozan),                                
     *     xxxx(lfmomas+ixftch(xxxx(lindfrg), IEFMOCFRG)-1),ZMSSTN,2)   
                                                                        
      DO I=1,NPTSTN                                                     
       CALL EFMOSTOR(IEFMOCFRG,I,1,XXXX(LEFMOPTS),IEFMONFRG,STNPNT(2,I))
       CALL EFMOSTOR(IEFMOCFRG,I,2,XXXX(LEFMOPTS),IEFMONFRG,STNPNT(3,I))
       CALL EFMOSTOR(IEFMOCFRG,I,3,XXXX(LEFMOPTS),IEFMONFRG,STNPNT(4,I))
       CALL EFMOSTOR(IEFMOCFRG,I,4,XXXX(LEFMOPTS),IEFMONFRG,ZMSSTN(I))  
       CALL EFMOSTOR(IEFMOCFRG,I,5,XXXX(LEFMOPTS),IEFMONFRG,ZANSTN(I))  
       CALL EFMOSTOR(IEFMOCFRG,I,6,XXXX(LEFMOPTS),IEFMONFRG,CHG(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,7,XXXX(LEFMOPTS),IEFMONFRG,X(I))       
       CALL EFMOSTOR(IEFMOCFRG,I,8,XXXX(LEFMOPTS),IEFMONFRG,Y(I))       
       CALL EFMOSTOR(IEFMOCFRG,I,9,XXXX(LEFMOPTS),IEFMONFRG,Z(I))       
       CALL EFMOSTOR(IEFMOCFRG,I,10,XXXX(LEFMOPTS),IEFMONFRG,XX(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,11,XXXX(LEFMOPTS),IEFMONFRG,YY(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,12,XXXX(LEFMOPTS),IEFMONFRG,ZZ(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,13,XXXX(LEFMOPTS),IEFMONFRG,XY(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,14,XXXX(LEFMOPTS),IEFMONFRG,XZ(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,15,XXXX(LEFMOPTS),IEFMONFRG,YZ(I))     
       CALL EFMOSTOR(IEFMOCFRG,I,16,XXXX(LEFMOPTS),IEFMONFRG,XXX(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,17,XXXX(LEFMOPTS),IEFMONFRG,YYY(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,18,XXXX(LEFMOPTS),IEFMONFRG,ZZZ(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,19,XXXX(LEFMOPTS),IEFMONFRG,XXY(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,20,XXXX(LEFMOPTS),IEFMONFRG,XXZ(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,21,XXXX(LEFMOPTS),IEFMONFRG,XYY(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,22,XXXX(LEFMOPTS),IEFMONFRG,YYZ(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,23,XXXX(LEFMOPTS),IEFMONFRG,XZZ(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,24,XXXX(LEFMOPTS),IEFMONFRG,YZZ(I))    
       CALL EFMOSTOR(IEFMOCFRG,I,25,XXXX(LEFMOPTS),IEFMONFRG,XYZ(I))    
      ENDDO                                                             
      ENDIF                                                             
C                                                                       
      RETURN                                                            
...
C                                                                       
      USE MX_LIMITS,ONLY:MXFGPT,mxatm,mxao                              
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      DOUBLE PRECISION MAKEFP                                           
C                                                                       
...
C       EFMO IS ALLOWED TO RUN                                          
        IF (IEOF.EQ.1 .AND. IEFMORUN.GT.0 .AND. IEFMORT.EQ.1) GOTO 11   
...
C     DEFAULTS FOR EFMO                                                 
C                                                                       
      IF((ISTN.EQ.1 .AND. IEFMORUN.GT.0 .AND. IEFMORT.EQ.1 )) THEN      
        IF( ISTEP.EQ.1 ) THEN                                           
          TOKEN = ATOMS                                                 
          GOTO 150                                                      
        ENDIF                                                           
C       NO BONDS                                                        
        IF(IAND(IMODEFE,16).NE.0) THEN                                  
          IF(ISTEP.EQ.2) THEN                                           
            TOKEN = BONDS                                               
            GO TO 150                                                   
          ENDIF                                                         
...
C>                                                                      
C>    @brief    Driver routine for Stone's distributed multipole analysi
C>                                                                      
C>    @details  calls routines to calculate Stone's DMA and an LMO      
C>              expansion                                               
C>                                                                      
C>    @author   Jan Jensen? LVS?                                        
C>                                                                      
C>    @date     January, 2017 - C.Bertoni                               
C>              - Changes to store coefficients for EFMO gradients      
C>                                                                      
...
C                                                                       
      ...
      END IF                                                            
C                                                                       
      if( iefmorun .gt. 0 .and. iefmort .eq. 1                          
     *     .and. iefmo_agrad .gt. 0) then                               
      lotherc  =last                                                    
      lotherd  =lotherc+nat                                             
      lotherq  =lotherd+3*nat                                           
      ldsint   =lotherq+6*nat                                           
      lcontrib =ldsint+NUM2*3*nat                                       
      locc     =lcontrib+NUM*NUM*3*nat                                  
      last     =locc+NUM*NUM                                            
      end if                                                            
...
C                                                                       
      ...
      INDMP=0                                                           
CCC no or very low chance to parallelize                                
      DO 12 IMO=1,NA                                                    
         JMAX=NMOAT(IMO)                                                
         NKMPT=0                                                        
         DO 14 IAT=1,MXMID                                              
...
C                                                                       
         ...
         INDMO=0                                                        
CCC no or low chance to parallelize                                     
         DO 20 JMO=1,NA                                                 
            NDELMO=0                                                    
            DO 30 JAT=1,MXATM                                           
...
C     check with dipder                                                 
      if( iefmorun .gt. 0 .and. iefmort .eq. 1                          
     *     .and. iefmo_agrad .gt. 0) then                               
         call daread(idaf, ioda, X(locc), num3, 15, 0)                  
         nvir = num-na                                                  
c     calculate terms for the derivative. the indexing of x(locc) and   
c     x(locc+na*num) may seem a bit strange, but remember that it's     
c     column-major.                                                     
         call EFMO_MULTIPOLE_GRAD( x(ndm), num2,                        
     *      x(lotherc),x(lotherd),x(lotherq), x(lefmoesder),            
     *        iefmocfrg, iefmonfrg, x(ldsint), x(lcontrib), nvir,       
     *        x(locc), x(locc+na*num), x(lzvwrk), MXZVWK )              
      end if                                                            
C                                                                       
      N=NDM-1                                                           
CCC no or low chance to parallelize                                     
...
C       HERE WE RELY ON NPT41 HAVING A CONTINUOUS FLOW OF MULTIPOLES.   
        CALL XCOPY(NAT*(NFRGMUL+1),X(NPT41),1,X(LSTONEP),1)             
        CALL XSCAL(NAT,-1.0D+00,X(LSTONEP),1)                           
...
C                                                                       
      ...
      IMGABISV = IMGABI                                                 
      IMGABI   = 1                                                      
CCC possible, but risky to parallelize: lots of IO                      
      DO 200 LMN=1,NA                                                   
...
CCC easy to parallelize, but this is printing to screen, which we probab
CCC want to see in an ordered manner; for all WRITE(IP,????) below      
      DO I=1,NA                                                         
...
C                                                                       
      CALL FLSHBF(IW)                                                   
C  ANGULAR GRID                                                         
      CALL LEBPTW(XYZ,XYZLEB,W,N_A,MXDIM,.TRUE.)                        
C                                                                       
...
C*MODULE PRPPOP  *DECK RADWFN2                                          
     ...
     *                   NSHELL,IDEGSH,MEMSH,RINC,NTICKS,PRTRAD)        
      use mx_limits, only: mxao,mxrt,mxnoro                             
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
...
C                                                                       
      ...
      LOGICAL PRTRAD,GOPARR,DSKWRK,MASWRK,                              
     *        CANONC,FCORE,FORS,EKT,LINSER                              
C                                                                       
C                                                                       
...
C     IN THE DEFINITION OF THE LOCAL HARDNESS. GAMESS ASSUMES NO SUCH   
...
C                                                                       
C     GET THE POTENTIAL ON ATOMS                                        
C                                                                       
      CALL EPCALC(C,X(IPOT),X(IDENSA),NAT,L2)                           
C     WRITE(6,*) 'WWWPOTV',(-X(IPOT+I-1),I=1,NAT)                       
C                                                                       
C     GET THE POPULATIONS                                               
C                                                                       
      CALL DAREAD(IDAF,IODA,X(IPOP),NAT,493,0)                          
C                                                                       
C     COMPUTE ATOMIC HARDNESS                                           
C                                                                       
      PI=ACOS(ZERO)*2                                                   
      CF=THREE/10*(3*PI*PI)**(2/THREE)                                  
      CX=THREE/4*SQRT(3/PI)                                             
C     WRITE(6,*) 'WWWCC',CF,CX                                          
      CKIN=TEN/(9*NE)*CF                                                
      CES=-ONE/NE                                                       
      CEX=-4/(NINE*NE)*CX                                               
      IF(MASWRK) THEN                                                   
        WRITE(IW,9000)                                                  
        DO KAT=1,NAT                                                    
          ETA2=-CES*X(IPOT+KAT-1)                                       
C         THE MINUS BEFORE CES IS BECAUSE THE POTENTIAL IS CALCULATED   
C         WITHOUT THIS SIGN, WHICH SHOULD BE ADDED EXPLICITLY.          
C                                                                       
          EDENS = X(IPOP+KAT-1)                                         
          ETA1=CKIN*EDENS**(2/THREE)                                    
          ETA3=CEX*EDENS**(1/THREE)                                     
C         WRITE(6,*) KAT,EDENS,X(IPOT+KAT-1)                            
          WRITE(IW,9010) KAT,                                           
     *                   ETA1,ETA2,ETA3,                                
     *                   ETA1+ETA2+ETA3                                 
        ENDDO                                                           
        IF(IAHARD.EQ.1) WRITE(IW,9020)                                  
        IF(IAHARD.EQ.2) WRITE(IW,9021)                                  
        IF(IAHARD.EQ.3) WRITE(IW,9022)                                  
        WRITE(IW,9040) NE                                               
      ENDIF                                                             
      CALL RETFM(NEED)                                                  
      RETURN                                                            
 9000 FORMAT(/1X,'LOCAL HARDNESS, SEE EQ. 9 IN S. SAHA AND R. K. ROY,', 
     *           ' PCCP, 10, 5591, 2008.'                               
     *      /1X,'  IAT    KINETIC  ELECTROSTATIC  EXCHANGE',            
     *           '  TOTAL (A.U.)',/1X,67(1H-))                          
 9010 FORMAT(1X,I5,4F12.6)                                              
 9020 FORMAT(/1X,'BASED ON MULLIKEN POPULATIONS.')                      
 9021 FORMAT(/1X,'BASED ON LOWDIN POPULATIONS.')                        
 9022 FORMAT(/1X,'BASED ON PDC (ESP) POPULATIONS.')                     
