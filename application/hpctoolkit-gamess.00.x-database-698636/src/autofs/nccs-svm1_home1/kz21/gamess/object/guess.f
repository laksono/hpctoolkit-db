...
C 18 Apr 16 - DGF - Reduce output                                       
C 22 Oct 14 - DGF,YN - pad commons, add DFTB                            
C  1 Aug 14 - MWS - COPROJ: trust G/S to recover possible EVVRSP fails  
C 21 May 13 - DGF - PAD COMMONS FOR FMO 5.0                             
C 16 Apr 13 - MWS - Huckel uses 5d/7f in MBS, projects onto Q-matrix    
C                   of the true basis, fixed its orthormalization step. 
C 19 DEC 12 - JI  - Add h/i functionality to COQMAT,TSPHAR              
...
C  2 NOV 11 - MWS - SYMDNS: FIXES TO ACCOUNT FOR SPH.HARMONICS USED     
C  2 NOV 11 - MWS - GUESMO: RUNTYP=PROP ENFORCES REQUIREMENT OF MOREAD  
C 11 AUG 11 - MWS - PRINT BETTER MESSAGE FOR HUCKEL ATTEMPT BEYOND RN.  
C 15 APR 11 - MK  - GUESS=DMREAD COMPATIBLE WITH UHF                    
C 11 AUG 10 - DGF - SYNCH FMO COMMON BLOCK                              
C 23 JUN 10 - RP  - UPDATE COSMO INTERFACE                              
C 10 MAY 10 - AA  - PARALLELIZE, COPROJ GOES BACK TO EVVRSP (UNDO 1/09) 
C 25 MAR 10 - MK  - GUESS=MOREAD COMPATIBLE WITH NOSO, ARGS TO DCGUES   
C 14 OCT 09 - DGF - SYNCHRONISE FMOOPT                                  
C 22 MAY 09 - MWS - SYNCHRONIZE DFGRID COMMON                           
C 12 JAN 09 - MWS - HUCKLM: MO COUNTS IF ORMAS/GMCCI, COPROJ: USE JACOBI
C 15 DEC 08 - DGF - GUESS CHANGES FOR FMO AND COMMON PADDING            
C 20 NOV 08 - MK  - CHANGES TO ADD THE DIVIDE-AND-CONQUER METHOD        
C 11 APR 08 - MWS - SYNCHRONIZE DFGRID COMMON BLOCK                     
C  4 MAR 08 - HN  - INIDEN: FIX L1/L0 USAGES IN PRINTING                
C 20 AUG 07 - DGF - SYNCHRONISE FMOOPT AND FMOPNT                       
C 22 DEC 06 - DGF - SYNCHRONISE FMOOPT,FMOPNT                           
C  6 NOV 06 - MWS - ADJUST WAVEFUNCTION COMMON BLOCK                    
C  8 MAY 06 - MWS - COOVLP: ADJUSTED FOR H AND I SHELLS                 
C 14 NOV 05 - DGF - SYNCH ARGS TO PREVS                                 
C 19 SEP 05 - MWS - ADD TRUE NUCLEAR CHARGE ARRAY TO INFOA COMMON       
C  5 JUL 05 - MWS - SELECT NEW ATOM,BASIS,EFP,PCM,DAF DIMENSIONS        
C 30 APR 05 - DGF - ADD GUESS=FMO, SYNCHRONISE SYMBLK COMMON BLOCK      
C 13 FEB 05 - MWS - PAD COMMON BLOCK NSHEL, ROOT, EDCMP                 
C  5 FEB 05 - MWS - GUESMO: ZERO SWOFF OPTION IF MOREAD IS CHOSEN       
C 19 MAY 04 - DGF - CHANGES TO ADD THE FMO METHOD                       
C 16 JAN 04 - MWS - HUCKLP: START ON MCP CORE COUNT PROBLEMS (IN)       
...
C                   MKSPHAR: CHANGE TRANSFORMATION TO PURE F FUNCTIONS. 
...
C>                                                                      
C>    @brief    compute overlap of two different GTO basis sets         
C>                                                                      
C>    @details  compute overlap of two different GTO basis sets,        
C>              which can be the same basis set in case of MODE=0.      
C>              The basis sets can different by geometry, or by         
C>              their constitution, or both.                            
C>                                                                      
     ...
     *                  NGAUCO,NSHLCO,EXCO,                             
     *                  CSCO,CPCO,CDCO,CFCO,CGCO,CHCO,CICO,             
     *                  KSTRCO,KATMCO,KTYPCO,KNGCO,KLOCCO,KMINCO,KMAXCO,
     *                  C,CCO)                                          
C                                                                       
...
C                                                                       
     ...
     *          CF(NGAU),CG(NGAU),CH(NGAU),CI(NGAU),                    
     *          KSTART(NSHL),KATOM(NSHL),                               
     *          KTYPE(NSHL),KNG(NSHL),KLOC(NSHL),KMIN(NSHL),KMAX(NSHL), 
     *          EXCO(NGAUCO),CSCO(NGAUCO),CPCO(NGAUCO),CDCO(NGAUCO),    
     *          CFCO(NGAUCO),CGCO(NGAUCO),CHCO(NGAUCO),CICO(NGAUCO),    
     *          KSTRCO(NSHLCO),KATMCO(NSHLCO),                          
     *          KTYPCO(NSHLCO),KNGCO(NSHLCO),KLOCCO(NSHLCO),            
     *          KMINCO(NSHLCO),KMAXCO(NSHLCO),                          
     *          SMAT(L2),SMATCO(L1CO,L1),                               
     *          C(3,NATM),CCO(3,NATM)                                   
      DIMENSION DIJ(784),S(784),IJX(784),IJY(784),IJZ(784),             
...
C                                                                       
            ...
            DO 520 IG = I1,I2                                           
               IF(MODE.EQ.0) THEN                                       
                  ...
                  IF(LIT.EQ.6) CHI = CH(IG)                             
                  IF(LIT.EQ.7) CII = CI(IG)                             
                  AI = EX(IG)                                           
               ELSE                                                     
                  ...
                  IF(LIT.EQ.6) CHI = CHCO(IG)                           
                  IF(LIT.EQ.7) CII = CICO(IG)                           
                  AI = EXCO(IG)                                         
               END IF                                                   
               ARRI = AI*RR                                             
               AXI = AI*XI                                              
...
C                                                                       
               ...
               DO 500 JG = J1,JGMAX                                     
                  ...
                  IF(LJT.EQ.6) CHJ = CH(JG)                             
                  IF(LJT.EQ.7) CIJ = CI(JG)                             
                  AX = (AXI+AJ*XJ)*AA1                                  
                  AY = (AYI+AJ*YJ)*AA1                                  
                  AZ = (AZI+AJ*ZJ)*AA1                                  
C                                                                       
...
C                                                                       
                     ...
                     IF((I.EQ.75).AND.NORM) DUM1 =                      
     *                                      DUM1*SQRT7/(SQRT3*SQRT5)    
...
C                                                                       
                        IF(J.EQ.1) THEN                                 
                          DUM2 = DUM1*CSJ                               
                          IF(DOUBLE) THEN                               
                             IF(I.EQ.1) DUM2 = DUM2 + DUM2              
                             IF(I.GT.1) DUM2 = DUM2 + CSI*CPJ*FAC       
                          END IF                                        
C                                                                       
                        ELSE IF( J.EQ.2) THEN                           
                          DUM2 = DUM1*CPJ                               
...
C>                                                                      
C>    @brief   project minimal basis set orbitals onto true basis       
C>                                                                      
C>    @details this is accomplished by a single value decomposition,    
C>             albeit by squaring D in order to use an ordinary         
C>             diagonalization call.                                    
C>                                                                      
C>    @author  Mike Schmidt, circa 1991                                 
C>                                                                      
      SUBROUTINE COPROJ(VB,SBA,VA,VACO,WRK,D,S,SCR,EIG,IWRK,            
     *                  NDOC,NACT,NPROJ,L1,L1CO,L0,L2,IW,DBG)           
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL DBG                                                       
C                                                                       
      DIMENSION VB(L1CO,NPROJ),SBA(L1CO,L1),VA(L1,L0),VACO(L1,L1),      
     *          WRK(L1,L1),D(NPROJ,L0),S(L2),SCR(L1,8),EIG(L1),         
     *          IWRK(L1)                                                
...
C     -L0- ORBITALS WITHIN THE -VA- ORBITAL SPACE.                      
C     -VA- AND -VB- CONTAIN THE -A- AND -B- ORBITALS ON ENTRY,          
C     -SBA- CONTAINS THE OVERLAP INTEGRALS BETWEEN THE TWO BASES,       
C     -VB- AND -SBA- WILL BE UNCHANGED ON EXIT, WHILE                   
C     -VA- IS REPLACED BY THE CORRESPONDING ORBITALS.                   
C     THE CORRESPONDING ORBITALS IN THE -B- BASIS ARE NOT GENERATED.    
C                                                                       
...
C                                                                       
      IF(DBG) THEN                                                      
         ...
         WRITE(IW,*) 'THE -B- MBS ORBITALS ARE'                         
...
C        VB-DAGGER * SBA                                                
C                                                                       
      CALL XGEMM('T','N',NPROJ,L1,L1CO,ONE,VB,L1CO,SBA,L1CO,ZERO,WRK,L1)
C                                                                       
...
C                                                                       
     ...
     *                     IPASS,MINMO,MAXMO,NMO,NREST                  
            ...
            CALL PRSQ(VA,L0,L1,L1)                                      
         END IF                                                         
C                                                                       
C        D = (VB-DAGGER*SAB) * VA                                       
C                                                                       
         CALL XGEMM('N','N',NMO,NREST,L1,ONE,WRK(MINMO,1),L1,           
...
C                                                                       
         IJ = 1                                                         
         DO I=1,NREST                                                   
            CALL XGEMV('T',NMO,I,ONE,D,NPROJ,D(1,I),1,ZERO,S(IJ),1)     
            IJ = IJ + I                                                 
         ENDDO                                                          
C                                                                       
         IF(DBG) THEN                                                   
...
C                                                                       
C     JAN 2009: THIS MATRIX HAS MANY ZERO EIGENVALUES, AND THE HIGH     
C     DEGREE OF DEGENERACY SOMETIMES CAUSES EVVRSP TO FAIL.  THEREFORE  
C     USE OF A JACOBI PROCEDURE IS FORCED.                              
C                                                                       
C     APRIL 2010: MWS HAD DECIDED TO RUN THIS PARTICULAR DIAGONALIZATION
C     WITH JACOBI, DUE TO ITS HAVING MANY ZERO EIGENVALUES.  JACOBI     
C     CAUSES RANDOM TIMINGS IN PARALLEL, AND ALSO MISSES THE PARALLEL   
C     CONTENT ADDED TO EVVRSP IN APRIL 2010 BY ANDREY.  HENCE THIS      
C     IS PUT BACK TO DEFAULT USER INPUT FOR -KDIAG-.  NOTE THERE ARE    
C     NEW ORTHOGONALIZATION PROCEDURES INSIDE EVVRSP AS OF 4/2010.      
C                                                                       
         KDIAG_SAVE = KDIAG                                             
         if(iand(modio,65536).ne.0) KDIAG=3                             
C                                                                       
C        EIGENVALUES RUN 0 TO 1, WE WANT THE HIGHEST EIGENVECTORS FIRST,
C        WHICH IS EASILY ACCOMPLISHED BY A SIGN CHANGE.  NOTE THAT WE   
C        DON'T USE THE VALUES, SO NEVER NEED TO FLIP THEIR SIGNS BACK.  
C                                                                       
         CALL XSCAL(L2,-ONE,S,1)                                        
         CALL GLDIAG(L1,NREST,NREST,S,SCR,EIG,VACO,IERR,IWRK)           
C                                                                       
         KDIAG=KDIAG_SAVE                                               
C                                                                       
C        THE NEAR ZERO OVERLAP PART OF THE SPACE MAY BE SCRAMBLED       
C        DURING THE ABOVE DIAGONALIZATION, BECAUSE OF THE HIGH          
C        DEGENERACY AROUND ZERO.  WE CAN ALSO GET -NAN- VALUES          
C        AS WELL, IN ORBITALS WE DON'T REALLY WANT.  IN 7/2014,         
C        STOPPED ABORTING THE JOB IN CASE OF BAD VECTORS,               
C        ON THE ASSUMPTION THEY ONLY OCCUR IN THE UNOCCUPIED SPACE.     
C                                                                       
C---  LOGICAL GOPARR,DSKWRK,MASWRK                                      
C---  COMMON /PAR   / ME,MASTER,NPROC,IBTYP,IPTIM,GOPARR,DSKWRK,MASWRK  
C---     IF(IERR.NE.0) THEN                                             
C---        IF (MASWRK) WRITE(IW,*)                                     
C--- *            'UNABLE TO DIAGONALIZE IN COPROJ'                     
C---        CALL ABRT                                                   
C---        STOP                                                        
C---      END IF                                                        
C                                                                       
C        WE CAN GET A SYMMETRY ADAPTED VIRTUAL SPACE BY GRAM-SCHMIDT    
C        ORTHOGONALIZATION, AUTOMATICALLY FIXING ANY POOR VECTORS.      
C                                                                       
         CALL SCHMD(VACO,NMO,NREST,L1,SCR)                              
C                                                                       
         IF(DBG) THEN                                                   
...
C                                                                       
         DO I=MINMO,MAXMO                                               
            CALL XGEMV('T',L1,NMO,ONE,VACO,L1,WRK(I,1),L1,ZERO,EIG,1)   
            CALL XCOPY(NMO,EIG,1,WRK(I,1),L1)                           
         ENDDO                                                          
C                                                                       
...
C                                                                       
         CALL XCOPY(NREST*L1,VACO,1,VA(1,MINMO),1)                      
  700 CONTINUE                                                          
      RETURN                                                            
C                                                                       
...
C>                                                                      
C> @brief   generate Q matrix for real basis during Huckel guess        
C>                                                                      
C> @details generate Q matrix for real basis during Huckel guess        
C>                                                                      
      SUBROUTINE COQMAT(Q,WRK,S,SCR,EIG,IWRK,L0,L1,L2,L3)               
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      DIMENSION Q(L1,L1),WRK(L1,L1),S(L2),SCR(L1,8),EIG(L1),IWRK(L1)    
C                                                                       
      COMMON /IOFILE/ IR,IW,IP,IJK,IJKT,IDAF,NAV,IODA(950)              
C                                                                       
C        ----- GENERATE SYMMETRY ADAPTED CANONICAL ORBITALS -----       
C        this is the genuine Q-matrix for this geometry, with           
C        symmetry adapted linear combinations made, and perhaps         
C        contaminations for non-spherical harmonics removed.            
C        L0 will also account for possible removal of linearly          
C        dependent MOs.                                                 
C                                                                       
C        The entire usable basis, e.g. spdfghi, is spanned by Q.        
C                                                                       
C        The Q matrix will be the target for the corresponding          
C        orbital projection of the extended Huckel MBS orbitals.        
C                                                                       
      CALL DAREAD(IDAF,IODA,S,L2,12,0)                                  
      CALL QMTSYM(S,WRK,Q,EIG,SCR,IWRK,L0,L1,L2,L3,.FALSE.)             
      CALL DAWRIT(IDAF,IODA,Q,L3,45,0)                                  
      RETURN                                                            
      END                                                               
C*MODULE GUESS   *DECK GESMEM                                           
C>                                                                      
C>    @brief   select type of initial orbital guess                     
C>                                                                      
C>    @details select type of initial orbital guess                     
C>                                                                      
      SUBROUTINE GESMEM(NORB,GUESS,NORDER,IORDER,JORDER,TOLZ,TOLE,MIX,  
     *                  PRTMO,PUNMO,SYMDEN,PURIFY,INSORB,masout)        
      use mx_limits, only: mxatm                                        
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL DBG,MIX,PRTMO,PUNMO,SYMDEN,PURIFY,QUICK,                  
     *        GOPARR,DSKWRK,MASWRK,READ,masout                          
...
C                                                                       
      ...
      DATA CORE/8HHCORE   /, GHCK/8HHUCKEL  /,FRAGMO/8HFMO     /        
...
C                                                                       
     ...
     *            X(LIWRK),NAT,L1,L2,L3,DBG,QUICK,NEED,READ,masout)     
     ...
     *            X(LH),X(LQ),X(LVEC),L1,L2,L3,DBG,NB)                  
      IF(GUESS.EQ.FRAGMO) THEN                                          
             CALL MOFRG(SCFTYP,X(LIWRK),X(LEIG),X(LSCR),                
     *            X(LH),X(LQ),X(LVEC),L1,L2,L3,DBG,NB)                  
             GOTO 1000                                                  
      ENDIF                                                             
      IF(GUESS.EQ.RDDM) THEN                                            
C        READ IN DENSITY MATRIX FROM INPUT FILE                         
             LDA = LH                                                   
             LDB = LS                                                   
             CALL READDM(X(LDA),X(LDB),L1,L2,DBG,SCFTYP.EQ.UHF)         
             CALL DAWRIT(IDAF,IODA,X(LDA),LL2,16,0)                     
             IF(SCFTYP.EQ.UHF) CALL DAWRIT(IDAF,IODA,X(LDB),LL2,20,0)   
C        GET CANONICAL ORBITALS                                         
             CALL DAREAD(IDAF,IODA,X(LS),LL2,12,0)                      
             CALL QMTSYM(X(LS),X(LVEC),X(LQ),X(LEIG),X(LSCR),X(LIWRK),  
     *                   L0,L1,L2,L3,DBG)                               
             CALL DAWRIT(IDAF,IODA,X(LQ),LL3,45,0)                      
             GOTO 1000                                                  
      ENDIF                                                             
C                                                                       
C        ----- GENERATE "CLEAN" MOLECULAR ORBITALS -----                
C     WE HAVE EXECUTED THE CODE BELOW EVER SINCE 1988.                  
C     IN APRIL 2011, THIS WAS FOUND TO PERTURB THE DEGREE OF            
C     ORBITAL ORTHOGONALITY, SO WE WILL TRY DOING WITHOUT IT.           
C                                                                       
C---      CALL DAREAD(IDAF,IODA,X(LVEC),LL3,15,0)                       
C---      CALL CLENMO(X(LVEC),L1,L1,TOLZ,TOLE,IW,DBG)                   
C---      CALL DAWRIT(IDAF,IODA,X(LVEC),LL3,15,0)                       
C---      IF(SCFTYP.EQ.UHF) THEN                                        
C---         CALL DAREAD(IDAF,IODA,X(LVEC),LL3,19,0)                    
C---         CALL CLENMO(X(LVEC),L1,L1,TOLZ,TOLE,IW,DBG)                
C---         CALL DAWRIT(IDAF,IODA,X(LVEC),LL3,19,0)                    
C---      END IF                                                        
C                                                                       
C        ----- HOMO/LUMO MIXING -----                                   
C                                                                       
      IF (MIX) THEN                                                     
         ...
         IF (MASout) WRITE(IW,9001)                                     
      END IF                                                            
C                                                                       
C-----------EXTRA CRISPY ORTHOG-------                                  
C     IF(MASWRK) WRITE(6,*) 'TEST ORTHOGONALITY: NUM,NQMT=',NUM,NQMT    
C     L0 = NQMT                                                         
C     CALL VERIFY_ONB(X(LVEC),X(LQ),X(LS),X(LH),X(LSCR),L0,L1,L2)       
C     IF(MASWRK) WRITE(6,*) 'PULLING THE PLUG...'                       
C     CALL ABRT                                                         
C-----------EXTRA CRISPY ORTHOG-------                                  
C                                                                       
...
C                                                                       
     ...
     *            SYMDEN,PURIFY,NA1,1,LL1,LL2,LL3,masout)               
 1000 CONTINUE                                                          
      CALL RETFM(NEED)                                                  
C                                                                       
      RETURN                                                            
...
C*MODULE GUESS   *DECK GUESMO                                           
      ...
      use mx_limits, only: mxatm,mxao,mxgrid,mxsh                       
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL VALID,MIX,PRTMO,PUNMO,SYMDEN,PURIFY,MFRZ,                 
     *        GOPARR,DSKWRK,MASWRK,SG1                                  
...
C                                                                       
     ...
     *      RDMINI/8HRDMINI  /,FRAGMO/8HFMO     /                       
      DATA  RDDM/8HDMREAD  /, HUCSUB/8HHUCSUB  /                        
      DATA PROP/8HPROP    /                                             
C                                                                       
...
C                                                                       
     ...
     *   0,0,0,0,0,0)                                                   
      ...
      END IF                                                            
      masout=maswrk                                                     
      if(nfg.ne.0) masout=maswrk.and.iand(nprfmo,3).lt.2                
C                                                                       
     ...
     * .OR. GUESS.EQ.RDMINI .OR. GUESS.EQ.FRAGMO                        
      VALID=VALID .OR. GUESS.EQ.RDDM                                    
      VALID=VALID .OR. (GUESS.EQ.HUCSUB .AND. DCFLG)                    
      IF(.NOT.VALID) THEN                                               
      ...
      END IF                                                            
C                                                                       
C         NO NEED FOR PRELIMINARY HF ITERATIONS AT FIRST DFT GEOMETRY   
C         IF WE ARE BEING GIVEN DECENT ORBITALS ALREADY.                
      IF(GUESS.EQ.RDMO  .OR.  GUESS.EQ.DAFMO  .OR.                      
     *   GUESS.EQ.SVMO) SWOFF=0.0D+00                                   
C                                                                       
      IF (MASout) WRITE (IW,910) GUESS,NORB,NORDER,  MIX,PRTMO,PUNMO,   
     *                           TOLZ,TOLE,   SYMDEN,PURIFY             
      IF (MASout .AND. INSORB.GT.0)  WRITE(IW,915) INSORB               
      GUESVL = GUESS                                                    
C                                                                       
      IF(RUNTYP.EQ.PROP  .AND.  GUESS.NE.RDMO) THEN                     
         IF(MASWRK) WRITE(IW,*) 'RUNTYP=PROP REQUIRES GUESS=MOREAD'     
         CALL ABRT                                                      
         STOP                                                           
      END IF                                                            
C                                                                       
...
C                                                                       
      ...
      IF(DFTBFL.AND.(GUESS.NE.RDMO)) THEN                               
c       VARIABLES FOR ORBITAL LOCALIZATION                              
        NOUTA = 0                                                       
        IF (MASout) THEN                                                
          WRITE (IW,*)                                                  
          WRITE (IW,*) "INITIAL GUESS OPTIONS ARE IGNORED FOR DFTB"     
        END IF                                                          
c       RETURN IF C1 SYMMETRY.  DFTB DOES NOT NEED INITIAL GUESS,       
c       BUT SYMMETRIC LABEL IS SOMETIMES USEFUL.                        
        IF (NT.EQ.1) RETURN                                             
        IF (MASout)                                                     
     *    WRITE (IW,*) "PROGRAM USES HCORE TO GET SYMMETRY INFORMATION" 
        GUESS=CORE                                                      
C       RETURN                                                          
      ELSE IF (DFTBFL) THEN                                             
        NOUTA = 0                                                       
      END IF                                                            
      IF(DCFLG) THEN                                                    
         CALL GUESDC(NORB,GUESS,NORDER,                                 
     *               TOLZ,TOLE,MIX,PRTMO,PUNMO,SYMDEN,PURIFY,INSORB)    
      ELSE                                                              
         CALL GESMEM(NORB,GUESS,NORDER,IORDER,JORDER,TOLZ,TOLE,MIX,     
     *               PRTMO,PUNMO,SYMDEN,PURIFY,INSORB,masout)           
      ENDIF                                                             
C                                                                       
...
C                                                                       
     ...
     *       CALL FRZLMX                                                
      ...
      RETURN                                                            
C                                                                       
  900 FORMAT(39H ERROR IN $GUESS NAMELIST INPUT - STOP )                
  910 FORMAT(/10X,13(1H-)/10X,13HGUESS OPTIONS/10X,13(1H-)/             
  ...
  930 FORMAT(/40H ERROR IN INITIAL GUESS OPTION, GUESS = ,A8)           
      END                                                               
...
C>                                                                      
C>    @brief   extended Huckel calculation in a minimal basis set       
C>                                                                      
C>    @details extended Huckel calculation in a minimal basis set,      
C>             including core orbitals!                                 
C>                                                                      
C>    @author  Mike Schmidt, circa 1991                                 
C>             extensively modified in 2013 to perform the extended     
C>             Huckel MBS calculation in a spherical harmonic basis.    
C>                                                                      
      SUBROUTINE HUCKEL(LLIM,IULIM,IA,EIG,H,S,WRK,VEC,TSH,Q,SCR,        
     *                  IWRK,DBG,MXGAU,MXSHL,EX,CS,CP,CD,CF,            
     *                  KSTART,KATOM,KTYPE,KNG,KLOC,KMIN,KMAX,          
     *                  L1CO,L2CO,L3CO,NGAUCO,NSHLCO,L0CO,NATM,         
     *                  NCELHW,USEIAN)                                  
      use mx_limits, only: mxatm                                        
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL DBG,NOTSP,NOTECP,GOPARR,DSKWRK,MASWRK,USEIAN,ONBDIAG      
C                                                                       
C                 -WRK- is used as both triangle and square of L1CO.    
      DIMENSION IA(L1CO),EIG(L1CO),TSH(L3CO),H(L2CO),S(L2CO),           
     *          VEC(L1CO,L1CO),Q(L1CO,L1CO),                            
     *          WRK(L1CO,L1CO),SCR(L1CO,8),IWRK(L1CO),                  
     ...
     *          LLIM(NATM),IULIM(NATM),NCELHW(104)                      
      DIMENSION ENEG(18),LNEG(56,6),NDVAL(4),NUMSPH(8)                  
C                                                                       
C                                                                       
...
C                                                                       
      ...
      PARAMETER (BITSY=0.05D+00)                                        
C                                                                       
C            order of MINI basis set must be exactly...     for  KIND   
C        1S,2S,2P,3S,3P,3D,4S,4P,4D, 5S,5P,5D,6S,6P         H-LA   1    
C        1S,2S,2P,3S,3P,3D,4S,4P,4D, 5S,5P,4F,5D,6S        CE-YB   2    
C        1S,2S,2P,3S,3P,3D,4S,4P,4D, 4F,5S,5P,5D,6S,6P     LU-RN   3    
C        .........................4F,5S,5P,5D,6S,6P,7S     FR-RA   4    
C        ......................4F,5S,5P,5D,6S,6P,6D,7S     AC-TH   5    
C        ...................4F,5S,5P,5D,6S,6P,5F,6D,7S     PA-LR   6    
C                                                                       
      DATA LNEG/1,  2,3*3,  4,3*5,5*6,  7,3*8,5*9,                      
     *              10,3*11,5*12,  13,3*14,           20*0,             
     *          1,  2,3*3,  4,3*5,5*6,  7,3*8,5*9,                      
     *              10,3*11,7*12,5*13,  14,           16*0,             
     *          1,  2,3*3,  4,3*5,5*6,  7,3*8,5*9,                      
     *              7*10,11,3*12,5*13,  14,3*15,      13*0,             
     *          1,  2,3*3,  4,3*5,5*6,  7,3*8,5*9,7*10,                 
     *             11,3*12,5*13, 14,3*15, 16,         12*0,             
     *          1,  2,3*3,  4,3*5,5*6,  7,3*8,5*9,7*10,                 
     *             11,3*12,5*13, 14,3*15, 5*16, 17,    7*0,             
     *          1,  2,3*3,  4,3*5,5*6,  7,3*8,5*9,7*10,                 
     *             11,3*12,5*13, 14,3*15, 7*16,  5*17, 18 /             
C                    8th shell is type L=SP                             
      DATA NUMSPH/1,3,5,7,9,11,13,4/                                    
C                                                                       
C     ----- CARRY OUT AN EXTENDED HUCKEL CALCULATION -----              
C     ROUTINE WRITTEN BY MIKE SCHMIDT AT NDSU: AUG. 2, 1985.            
C     MODIFIED TO COMPUTE THE GUESS IN A HUZINAGA MINIMAL BASIS SET,    
C     THESE MO'S ARE LATER PROJECTED ONTO THE TRUE BASIS: JULY 1991.    
C     MODIFIED TO RUN THE EXTENDED HUCKEL CALCULATION IN A SPHERICAL    
C     HARMONIC FORM OF THE MINIMAL BASIS SET: MWS, APRIL 2013.          
C                                                                       
      NCORE = 0                                                         
      NVAL = 0                                                          
C                                                                       
...
C           counting is done in terms of spherical harmonics.           
C                                                                       
      DO IAT=1,NAT                                                      
         IF(IAT.EQ.1) THEN                                              
            LLIM(IAT) = 1                                               
         ELSE                                                           
            LLIM(IAT) = IULIM(IAT-1) + 1                                
         END IF                                                         
         NAOSPH=0                                                       
         DO ISH=1,NSHLCO                                                
            KAT = KATOM(ISH)                                            
...
C                                                                       
     ...
     *            C,C)                                                  
C                                                                       
C        transform the overlap matrix to spherical harmonic form.       
C                                                                       
      CALL MKSPHAR(TSH,L1CO,L0CO,NOTSP,KTYPE,KLOC,KMIN,KMAX,NSHLCO)     
      IF(NOTSP) THEN                                                    
         CALL TFTRI(H,S,TSH,SCR,L0CO,L1CO,L1CO)                         
         CALL XCOPY(L2CO,H,1,S,1)                                       
      END IF                                                            
C                                                                       
C        obtain canonical orthonormal MOs -Q- for the minimal basis set.
C                                                                       
      CALL XCOPY(L2CO,S,1,H,1)                                          
      CALL GLDIAG(L1CO,L0CO,L0CO,H,SCR,EIG,Q,IERR,IWRK)                 
      DO I=1,L0CO                                                       
         FACT = ONE/SQRT(EIG(I))                                        
         CALL XSCAL(L1CO,FACT,Q(1,I),1)                                 
      ENDDO                                                             
C                                                                       
C        construct the extended Huckel operator -H- directly on top of  
C        a copy of the overlap -S- in spherical harmonic space.         
C                                                                       
      CALL XCOPY(L2CO,S,1,H,1)                                          
      DO 390 N=1,NAT                                                    
         NUCZ = INT(ZAN(N)) + IZCORE(N)                                 
         IF(USEIAN.AND.INT(ZAN(N))+1.EQ.IAN(N)) NUCZ=IAN(N)+IZCORE(N)   
C                                                                       
C           bond functions may sit on atoms with no nuclear charge, or  
C           sparkles with fractional nuclear charge may be present.     
C           We didn't generate a MBS for these, so here just need to    
C           avoid doing anything with them.                             
...
C                                                                       
C        count any core orbitals removed by a pseudo-potential,         
C        IC accounts for any semi-core orbitals not removed by the PP.  
C                                                                       
         IC = IZCORE(N)/2 - NCECP                                       
C                                                                       
C        if a core orbital is being removed by use of a ECP,            
C        that orbital is assigned a positive energy.                    
C                                                                       
         DO 320 I=1,NCORE                                               
            IROW=I0+I                                                   
            II=IA(IROW)+IROW                                            
...
C                                                                       
         ...
         IF(NEXTRA.GT.0) THEN                                           
            IF(MASWRK) THEN                                             
               WRITE(IW,9200)                                           
               WRITE(IW,9010) (I,LLIM(I),IULIM(I),I=1,NAT)              
            END IF                                                      
            CALL ABRT                                                   
         END IF                                                         
  390 CONTINUE                                                          
C                                                                       
C        In view of the very large core orbital energies,               
C        scale down all core/core and core/valence overlaps,            
C        to reduce the amount of mixing of these types.                 
C           IWRK=1,2 means core,valence function.                       
C        Because of the large range of parameters, one could            
C        consider making BITSY a function of the two diagonal           
C        elements of H, or different for C/C versus C/V overlaps.       
C                                                                       
      LOC=0                                                             
      DO I=1,L0CO                                                       
         ITYP = IWRK(I)                                                 
         DO J=1,I                                                       
            ...
            IF(I.NE.J) THEN                                             
               JTYP = IWRK(J)                                           
               IF(ITYP.NE.JTYP)            H(LOC)=BITSY*H(LOC)          
               IF(ITYP.EQ.1.AND.JTYP.EQ.1) H(LOC)=BITSY*H(LOC)          
            END IF                                                      
         ENDDO                                                          
      ENDDO                                                             
C                                                                       
C         generate the off-diagonal of the extended Huckel operator     
C                                                                       
      FUDGE = 1.75D+00/TWO                                              
      II = 1                                                            
      IJ = 1                                                            
...
C                                                                       
      ...
      END IF                                                            
C                                                                       
C         Add a contribution to the extended Huckel operator            
C         corresponding to the covalent bond detachment in FMO.         
C         This is no longer correctly programmed (as of 2013),          
C         since the original one expected H and S to be in the          
C         Cartesian form of the MBS.  This is invoked only by           
C         NGUESS=4 in $FMOPRP, which is a little used option.           
C         The USEIAN option is turned off elsewhere in this file.       
C                                                                       
      IF(USEIAN) CALL PROHUC(L1CO,H,VEC,Q,S,WRK,SCR,NSHLCO,KATOM,       
     *                       KTYPE,KLOC,KMIN)                           
C                                                                       
C         ----- diagonalize the extended Huckel operator -----          
C         This theory normally pretends there is no metric here,        
C         although it uses the metric to define its operator!           
C                                                                       
C         The corresponding orbital projection onto the true basis      
C         requires orthogonality on both sides of the single value      
C         decomposition, so the extended Huckel orbitals are sent       
C         through a Gram-Schmidt orthonormalization.                    
C         Alternatively, one can diagonalize in an orthonormal          
C         space, as shown, but this leads to worse starting energies.   
C                                                                       
      ONBDIAG=.FALSE.                                                   
C                     -wrk- used as a triangle here...                  
      IF(ONBDIAG) THEN                                                  
         CALL TFTRI(WRK,H,Q,SCR,L0CO,L0CO,L1CO)                         
         CALL XCOPY(L2CO,WRK,1,H,1)                                     
      END IF                                                            
C                                                                       
      IERR = 0                                                          
      CALL GLDIAG(L1CO,L0CO,L0CO,H,SCR,EIG,WRK,IERR,IA)                 
      IF(IERR.NE.0) THEN                                                
         IF (MASWRK) WRITE(IW,*) 'HUCKEL MBS DIAG FAILURE'              
         CALL ABRT                                                      
         STOP                                                           
      END IF                                                            
C                                                                       
C         orthonormalize appropriately.                                 
C                                                                       
      IF(ONBDIAG) THEN                                                  
         CALL TFSQB(WRK,Q,SCR,L0CO,L0CO,L1CO)                           
      ELSE                                                              
         CALL ORTHO(Q,S,WRK,SCR,L0CO,L0CO,L1CO,L2CO,L1CO)               
         CALL TFSQB(WRK,Q,SCR,L0CO,L0CO,L1CO)                           
      END IF                                                            
C                                                                       
C         backtransform to the Cartesian MBS.                           
C                                                                       
      IF(NOTSP) THEN                                                    
         CALL XGEMM('N','N',L1CO,L0CO,L0CO,ONE,TSH,L1CO,                
     *              WRK,L1CO,ZERO,VEC,L1CO)                             
      ELSE                                                              
         CALL XCOPY(L3CO,WRK,1,VEC,1)                                   
      END IF                                                            
C                                                                       
      CALL CLENMO(VEC,L1CO,L0CO,1.0D-06,1.0D-05,IW,.FALSE.)             
C                                                                       
      IF(DBG) THEN                                                      
         WRITE(IW,*) 'EXTENDED HUCKEL ORBITALS (IN MINI BASIS) ARE'     
         CALL PREVNL(VEC,EIG,L0CO,L1CO,L1CO)                            
      END IF                                                            
C                                                                       
      RETURN                                                            
C                                                                       
 ...
 9200 FORMAT(1X,'HUCKEL: CONFUSION WITH MINI BASIS SET')                
      END                                                               
C*MODULE GUESS   *DECK HUCKLB                                           
C>                                                                      
C>    @brief   extended Huckel minimal basis set                        
C>                                                                      
C>    @details The extended Huckel basis should include core orbitals,  
C>             to generate all electron starting orbitals, in which     
C>             case we use Huzinaga's MINI basis with 3 GTOs/AO         
C>             In some cases we switch to the HW ECP bases.             
C>                                                                      
C>    @author  Mike Schmidt, circa 1991                                 
C>                                                                      
...
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL GOPARR,DSKWRK,MASWRK,USEIAN                               
C                                                                       
      DIMENSION INTYP(MXSHL),NS(NATM),EX(MXGAU),                        
...
C                                                                       
      DO 120 N=1,NATSV                                                  
         ...
         IF(NUCZ.GT.86) THEN                                            
            IF(MASWRK) THEN                                             
               WRITE(IW,*) 'EXTENDED HUCKEL GUESS NOT AVAILABLE, AS'    
               WRITE(IW,*) 'MINIMAL BASIS SETS EXIST ONLY UP TO RN.'    
               WRITE(IW,*) 'PLEASE SWITCH TO GUESS=HCORE'               
            END IF                                                      
            CALL ABRT                                                   
         END IF                                                         
...
C>                                                                      
C>    @brief   perform extended Huckel initial orbital guess            
C>                                                                      
C>    @details calculation uses Huzinaga MINI basis, retaining cores:   
C>             1. HUCKLB: generate MINI basis set on the fly,           
C>                        this is sometimes a semi-core HW ECP basis.   
C>             2. HUCKEL: do extended Huckel in the MINI basis.         
C>             3. COOVLP: overlap between MINI and current run's basis. 
C>             4. COQMAT: make Q-matrix for the current run's basis.    
C>             5. COPROJ: project Huckel MOs onto current run's Q.      
C>             6. ORTHO/TFSQB: orthog. to make it all copacetic.        
C>    MOPAC does its Huckel elsewhere, and FMO may do extra stuff.      
C>                                                                      
C>    @author  Mike Schmidt, 1985 through 1991                          
C>             2013: use the Q-matrix as the molecular basis set        
C>             side of the single-value-decomposition projection.       
C>             2013: do the extended Huckel in spherical harmonics.     
C>                                                                      
      SUBROUTINE HUCKLM(LLIM,IULIM,EIG,H,S,VEC,Q,SCR,                   
     *                  IWRK,NATM,L1,L2,L3,DBG,QUICK,NEED1,READ,masout) 
      use mx_limits, only: mxgtot,mxsh,mxatm,mxrt,mxao,mxnoro           
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
      DOUBLE PRECISION METHOD                                           
C                                                                       
      LOGICAL DBG,QUICK,GOPARR,DSKWRK,MASWRK                            
      LOGICAL CANONC,FCORE,FORS,EKT,LINSER,READ,USEIAN,masout           
C                                                                       
C                                                                       
      DIMENSION LLIM(NATM),IULIM(NATM),IWRK(L1),EIG(L1),H(L2),S(L2),    
     *          VEC(L3),Q(L3),SCR(L1,8)                                 
      DIMENSION NCELHW(104),idamdt(3)                                   
C                                                                       
...
C                                                                       
      ...
      END IF                                                            
C                                                                       
C     USE JACOBI DIAGONALIZATION, IT AVOIDS SCRAMBLING DEGENERATE       
C     ORBITAL SYMMETRIES SOMEWHAT BETTER THAN GIVENS-HOUSEHOLDER.       
C     As of 12/2014, we continue to force Jacobi diags inside HUCKEL    
C     and COPROJ, but leave Q-matrix inside COQMAT up to user's choice. 
C                                                                       
      KDSAVE = KDIAG                                                    
      IF(NT.GT.1) KDIAG = 3                                             
C                                                                       
C         see note where PROHUC is called...                            
      USEIAN=NFG.NE.0.AND.NBDFG.NE.0.AND.MOD(NGUESS/4,2).EQ.1.AND.      
     *       RFLMO(1).NE.0                                              
      IF(USEIAN) THEN                                                   
...
C                                                                       
     ...
     *            L1CO,L2CO,L3CO,NGAUCO,NSHLCO,L0CO,NATM,NCELHW,USEIAN) 
C                                                                       
C        READ IN VEC                                                    
C                                                                       
      IF(READ) CALL RDMIMO(X(LVEC),L1CO)                                
C                                                                       
C        GET OVERLAP BETWEEN THE MINIMAL BASIS SET AND THE INPUT BASIS  
C                                                                       
      DUMMY=0.0D+00                                                     
      CALL COOVLP(1,DUMMY,X(LSCO),L1,L1CO,L2,NATM,                      
     *            MXGTOT,NSHELL,                                        
     *            EX,CS,CP,CD,CF,CG,CH,CI,                              
     *            KSTART,KATOM,KTYPE,KNG,KLOC,KMIN,KMAX,                
     *            NGAUCO,NSHLCO,X(LEX),X(LCS),X(LCP),                   
     *            X(LCD),X(LCF),DUMMY,DUMMY,DUMMY,                      
     *            X(LKSTAR),X(LKATOM),X(LKTYPE),                        
     *            X(LKNG),X(LKLOC),X(LKMIN),X(LKMAX),C,C)               
C                                                                       
C        GET "PURE" CANONICAL ORBITALS IN INPUT BASIS SPACE             
C        As of 2013, we generate this geometry's genuine Q-matrix,      
C        with L0 including anything dropped for linear dependence.      
C                                                                       
      KDIAG = KDSAVE                                                    
      CALL COQMAT(Q,X(LWRK1),S,SCR,EIG,IWRK,L0,L1,L2,L3)                
      IF(NT.GT.1) KDIAG = 3                                             
C                                                                       
...
C                                                                       
C        PROJECT MBS GUESS ONTO THE INPUT CANONICAL ORBITALS,           
...
C                                                                       
     ...
     *            L1,L1CO,L0,L2,IW,DBG)                                 
      CALL XCOPY(L1*NPROJ,Q,1,VEC,1)                                    
C                                                                       
C        GENERATE THE REMAINING VIRTUALS BY ORTHOGONALIZATION           
C                                                                       
      CALL DAREAD(IDAF,IODA,S,L2,12,0)                                  
      CALL DAREAD(IDAF,IODA,Q,L3,45,0)                                  
      CALL ORTHO(Q,S,VEC,SCR,NPROJ,L0,L1,L2,L1)                         
      CALL TFSQB(VEC,Q,SCR,L0,L1,L1)                                    
C                                                                       
C     PROJECT OUT BOND HMO FOR FMO                                      
C                                                                       
      IF(NFG.NE.0.AND.NBDFG.NE.0.AND.MOD(NGUESS/4,2).EQ.1.AND.          
     *   RFLMO(1).EQ.0) THEN                                            
C        WRITE(6,*) 'VECS BEFORE',L1                                    
C        CALL PRSQ(VEC,L1,L1,L1)                                        
C        SINCE SAVEH IS FALSE, Q IS NOT USED AND MAY BE DUMMY           
C        IWRK1 IS USED AS REAL, NOT INTEGER BUFFER OF L1 IN FMOHOP.     
         call viclr(idamdt,1,3)                                         
         CALL FMOHOP(L1,L2,VEC,S,X(LWRK1),Q,X(LDD),SCR,X(LWRK2),IWRK1,  
     *               X(LIABDFG),X(LJABDFG),X(LIDXCAO),X(LIAGLOB),       
     *               X(LNCBS),X(LNCAO),X(LIAPRJO),X(LJAPRJO),X(LSHIFTB),
     *               X(LCOREAO),X(LFMOC),X(LROTLCAO),X(LLOCFMO),NSHELL, 
     *             KATOM,KTYPE,KLOC,KMIN,.FALSE.,.FALSE.,rnone,idamdt)  
C        WRITE(6,*) 'VECS AFTER',NPROJ                                  
C        CALL PRSQ(VEC,L1,L1,L1)                                        
C        PROJECTION CREATES SOME LINEARLY DEPENDENT VIRTUAL ORBITALS    
C        SO WE REGENERATE VIRTUAL SPACE.                                
...
C                                                                       
 ...
 9200 FORMAT(1X,'SORRY, THE FMO/AFO OPTION IS PRESENTLY BROKEN'/        
     *       1X,'FOR ATOMS WITH OCCUPIED D,F ORBITALS,'/                
     *       1X,'SO THEREFORE THIS OPTION IS BEING TURNED OFF.'/        
     *       1X,'THIS IS NGUESS=4 IN $FMOPRP WITH NON-ZERO RFLMO.'/     
     *       1X,'PLEASE CONSIDER USING FMO/HOP, BY SETTING RFLMO ZERO.')
      END                                                               
C*MODULE GUESS   *DECK HUCKLP                                           
C>                                                                      
C>    @brief    return Huckel parameters for atom of charge NUCZ        
C>                                                                      
C>    @details  parameters are orbital energies, valence shell          
C>              info, and sometimes info about how to use a             
C>              minimal basis for semicore ECPs.                        
C>                                                                      
...
C        Semi-core HW minimal basis is used instead of Huzinaga MINI    
C        where possible: this simply isn't used for 2nd row atoms,      
C        and is unavailable for lanthanides, or for Zn,Cd,Hg,           
C        or for At/Rn.  This is basically assuming that the core        
C        potential is SBKJC type: no thought process here for MCP!      
C                                                                       
      NCECP = 0                                                         
C                                                                       
C           BOND FUNCTIONS                                              
...
C           IN-XE       VALENCE 5S,5P                                   
      IF(NUCZ.GE.49  .AND.  NUCZ.LE.54) THEN                            
         IF(NOTECP) THEN                                                
            NCORE=23                                                    
         ELSE                                                           
C              MOST ECP'S ARE SEMI-CORE FOR INDIUM, BUT NOT MCP'S       
            IF(NUCZ.EQ.49) THEN                                         
               IF(IECP.NE.5) THEN                                       
                  NCORE=9                                               
                  NCECP=14                                              
...
C           there are no 5d energy values stored in our table above.    
C           There's no HW basis set, except for La!                     
C           Usually we use MINI here, with 54 core electrons.           
...
C           LU-HG      VALENCE IS 5D,6S                                 
  ...
  640    CONTINUE                                                       
C               FOR THE TMs, WE ARBITRARILY MOVE 4F BELOW 5S.           
C               THIS IS MAINLY A BANDAID FOR THE SBKJC BASIS,           
C               WHERE 4F DISAPPEARS IN THE ECP,                         
C               SO ONLY 5S,5P ARE SEMICORE.                             
...
C           TL-RN       VALENCE IS 6S,6P                                
      ...
      END IF                                                            
C-------------------SEVENTH ROW-------------                            
C     note that Huzinaga basis sets stop at Rn so below isn't tested.   
C           FR-RA       VALENCE 7S                                      
...
C                                                                       
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)                               
C                                                                       
      LOGICAL SYMDEN,PURIFY,DBG,PRTMO,PUNMO,FRSTMO,masout,              
...
C GENERAL GVB CASE WOULD TAKE REORDERING TO MAKE PAIRS                  
      IF( FRSTMO  .AND. NPAIR.GT.1  .AND.  NORDER.EQ.0) THEN            
         IF(MASWRK) WRITE(IW,*) ' ERROR: GVB ORBITALS MUST BE ORDERED'  
         CALL ABRT                                                      
      END IF                                                            
      DO 470 I=1,NPAIR                                                  
...
C---        WRITE(IP,*) '$END'                                          
C---        CALL PREVS(V,E,LABMO,L1,L1,L1,1)                            
C---     END IF                                                         
      END IF                                                            
C                                                                       
...
C                                                                       
      ORBSET=' BOTH'                                                    
      IF (MASout) WRITE(IW,9020) ORBSET,NOC,NCORE                       
      IF(FRSTMO) THEN                                                   
         MINI=NCORE+1                                                   
      ...
      ELSE                                                              
         ...
         MAXI=MIN(L1,L0)                                                
      END IF                                                            
      IF (MASout) WRITE(IW,9022) (I,LABMO(I),I=MINI,MAXI)               
C                                                                       
      IF(MASWRK  .AND.  (DBG  .OR.  PRTMO)) THEN                        
         WRITE (IW,9026)                                                
         IF(FRSTMO) WRITE(IW,9046)                                      
         IF(FRSTMO) WRITE(IW,9048) (OCCNO(I),I = 1,L0)                  
...
C                                                                       
  600 CONTINUE                                                          
      ...
      NVEC = 15                                                         
      ORBSET='ALPHA'                                                    
      IF(SCFTYP.EQ.UHF) CALL XCOPY(L1,E,1,DB,1)                         
C                                                                       
      NPASS=0                                                           
  610 CONTINUE                                                          
      NPASS=NPASS+1                                                     
...
C                 GET READY TO DO THE BETA ORBITALS.                    
      ...
      ORBSET=' BETA'                                                    
      IF(SCFTYP.EQ.UHF) CALL XCOPY(L1,DB,1,E,1)                         
C                                                                       
...
C                                                                       
      ...
      END                                                               
C                                                                       
C*MODULE GUESS   *DECK MOFRG                                            
      SUBROUTINE MOFRG(SCFTYP,IWRK,EIG,SCR,S,Q,VEC,L1,L2,L3,DBG,NB)     
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL DBG                                                       
...
C                                                                       
      DATA UHF/8HUHF     /                                              
C                                                                       
C     ----- ZERO OUT THE INITIAL ORBITALS AND ORBITAL ENERGIES -----    
C       DENSITY WILL BE USED.                                           
C       THIS SHOULD WORK FOR: (RHF,ROHF,UHF)+(MPN(N=0,2),CIN).          
C       IT WILL NOT WORK FOR DFT,MCSCF OR GVB.                          
C                                                                       
C            GET CANONICAL ORBITALS                                     
C                                                                       
      CALL DAREAD(IDAF,IODA,S,L2,12,0)                                  
...
C                                                                       
  ...
  200 CONTINUE                                                          
      CALL DAWRIT(IDAF,IODA,VEC,L3,NSAV,0)                              
      CALL DAWRIT(IDAF,IODA,VEC,L1,NSAV+2,0)                            
C                                                                       
      IF(SCFTYP.NE.UHF) RETURN                                          
      IF(NB.EQ.0) RETURN                                                
      IF(NSAV.EQ.19) RETURN                                             
...
C                                                                       
      ...
      ENDIF                                                             
C                                                                       
      IF(PROOF) CALL XCOPY(L1*NMO,V,1,X(LV),1)                          
      BAIL=.TRUE.                                                       
...
C                                                                       
            DO 2000 IG=1,NIRRED                                         
              ...
              IRMON(I)=IG                                               
              BAIL=.FALSE.                                              
              CALL XCOPY(L1,X(LVI),1,X(LWORKV),1)                       
...
C               FOUND A NEW FUNCTION!                                   
                ...
                CALL XCOPY(L1,X(LVI),1,V(1,I),1)                        
                GOTO 3000                                               
              ENDIF                                                     
              CALL XCOPY(L1,X(LWORKV),1,X(LVI),1)                       
              IRMON(I)=ISAVE                                            
 2000       CONTINUE                                                    
C                                                                       
...
C> @brief      Reads in and checks the initial orbitals.                
C>                                                                      
...
C                                                                       
     ...
     *        DIPDCM,DEPRNT,QADDCM,ZDO,POLDCM,POLANG,POLAPP,            
     *        KMIDPT,POLDYN                                             
      CHARACTER*8 STVEC                                                 
      DOUBLE PRECISION MOROKM                                           
C                                                                       
...
C                                                                       
      ...
      DATA NONE/4HNONE/                                                 
C                                                                       
 9020 FORMAT(28H NO $VEC INPUT FOUND - STOP )                           
 ...
 9080 FORMAT(1X,'READMO: YOU FORGOT TO GIVE NORB IN $GUESS')            
  ...
  240    CONTINUE                                                       
     ...
     *                                                       IMAX+NSTM) 
            ...
            IF(JJ.EQ.MODJ . AND.  ICC.EQ.MODIC) GO TO 260               
               WRITE(IW,9060) J,MODJ,IC,JJ,ICC                          
...
C                                                                       
C  SAVE OUT INITIAL LCAO VECTORS FOR PRJSRF.                            
C                                                                       
      IF(RUNTYP.NE.MOROKM) CALL DAWRIT(IDAF,IODA,VEC,L3,555,0)          
C                                                                       
...
C        IF NOSO=1, S IS NO LONGER OVERLAP                              
         CALL DAREAD(IDAF,IODA,S,LL2,12,0)                              
...
C                                                                       
      DO 240 I = 1,NCOL                                                 
         J = IORDER(I)                                                  
         CALL XSWAP(LDV,V(1,I),1,V(1,J),1)                              
         IP1 = I+1                                                      
         DO 220 K = IP1,NCOL                                            
...
C                                                                       
      RETURN                                                            
      END                                                               
C*MODULE GUESS   *DECK MKSPHAR                                          
C                                                                       
C> @brief   Generate transformation to spherical harmonics basis        
C>                                                                      
C> @details This is used by the MINI basis during the Huckel guess,     
C>          to convert any d or f shells to spherical harmonics.        
C>                                                                      
C> @author  MWS: 4/2013 rewrite returns pure s,p,d,f spherical          
C>          harmonic transformation (contaminants now dropped).         
C>          This is used for occupied atoms, so does not need to        
C>          go beyond f orbitals for the known periodic table.          
C                                                                       
      SUBROUTINE MKSPHAR(W,L1CO,L0CO,NOTSP,KTYPE,KLOC,KMIN,KMAX,NSHELL) 
C                                                                       
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C                                                                       
      LOGICAL NOTSP                                                     
C                                                                       
      DIMENSION W(L1CO,L1CO),KTYPE(NSHELL),KLOC(NSHELL),                
     *          KMIN(NSHELL),KMAX(NSHELL)                               
C                                                                       
      PARAMETER (HALF=0.5D+00)                                          
      PARAMETER (ONE=1.0D+00)                                           
C                                                                       
C     ----- GENERATE TRANSFORMATION TO SPHERICAL HARMONIC BASIS -----   
C     This should only be called by a s,p,d,f basis set,                
C     such as the MINI basis used in the Huckel guess.                  
C     The transformation -W- returned is orthogonalizing within shells, 
C     namely W-DAGGER*S*W = I, for all S,P,D,F diagonal blocks.         
C     On any given atom, there will be no overlap between S,P,D,F       
C     orbitals, but intra-atomic SS, PP, DD, FF overlaps and of course  
C     overlaps with other atoms are still present.                      
C     Note: -W- is rectangular in shape, L1CO by L0CO, where -L0CO-     
C     returns the number of pure spherical harmonic AOs found.          
C                                                                       
      RT0304= SQRT( 3.0D+00/ 4.0D+00)                                   
      RT0920= SQRT( 9.0D+00/20.0D+00)                                   
C                                                                       
      CALL VCLR(W,1,L1CO*L1CO)                                          
C                                                                       
      KOL=1                                                             
      NCONT=0                                                           
      NOTSP = .FALSE.                                                   
      DO 200 N=1,NSHELL                                                 
         IF(KTYPE(N).GT.2) NOTSP=.TRUE.                                 
C                                                                       
C                   S,P,L shell is already spherical harmonics.         
         IF(KTYPE(N).LE.2) THEN                                         
            I = KLOC(N)                                                 
            NDIM = KMAX(N) - KMIN(N) + 1                                
            DO L=1,NDIM                                                 
              W(I+L-1,KOL+L-1) = ONE                                    
            ENDDO                                                       
            KOL=KOL+NDIM                                                
...
C                                                                       
         IF(KTYPE(N).EQ.3) THEN                                         
            I = KLOC(N)                                                 
C                          true D orbitals, from eg irrep of Oh         
            W(I  ,KOL  ) =  RT0304                                      
            W(I+1,KOL  ) = -RT0304                                      
            W(I  ,KOL+1) = -HALF                                        
            W(I+1,KOL+1) = -HALF                                        
            W(I+2,KOL+1) =  ONE                                         
C                          true D orbitals, from t2g irrep of Oh        
            W(I+3,KOL+2) =  ONE                                         
            W(I+4,KOL+3) =  ONE                                         
            W(I+5,KOL+4) =  ONE                                         
            NCONT=NCONT+1                                               
...
C                          A2U (IN OH) COMBINATION                      
         ...
         END IF                                                         
C                                                                       
         IF(KTYPE(N).GT.4) THEN                                         
            WRITE(6,*) 'MKSPHAR: CALLED FOR BASIS WITH G/H/I AOS'       
            CALL ABRT                                                   
         END IF                                                         
  200 CONTINUE                                                          
C                                                                       
...
C                                                                       
 ...
 9100 FORMAT(' *** ERROR: PREMATURE END OF ORBITAL INPUT ENCOUNTERED'/  
     *       '  (HUKMIX)  LOOKING FOR ORBITAL',I4,' LINE',I4)           
...
C                                                                       
      CALL XCOPY(NORB*L1,VEC2(1,1),1,VEC(1,1),1)                        
C                                                                       
...
9300  FORMAT(' *** ERROR: PREMATURE END OF ORBITAL INPUT ENCOUNTERED'/  
     *       '  (ADZERO)  LOOKING FOR ORBITAL',I4,' LINE',I4)           
C                                                                       
      STVEC = ' $VEC   '                                                
      NMOS=0                                                            
      NUMLV=0                                                           
...
C                                                                       
 ...
 9060 FORMAT(' *** ERROR READING THE MO COEFFICIENTS AT',2I4/           
     *       '     THE INPUT VALUES WERE',2I4)                          
 9100 FORMAT(' *** ERROR: PREMATURE END OF ORBITAL INPUT ENCOUNTERED'/  
     *       '  (RDMIMO)  LOOKING FOR ORBITAL',I4,' LINE',I4)           
C                                                                       
C     ----- READ THE INITIAL ORBITALS FROM THE INPUT FILE -----         
C                                                                       
      IZERO = 0                                                         
...
C                                                                       
      IF(LL1 .NE. L1) RETURN                                            
C                                                                       
C         ONE CAN IMAGINE REIMPLEMENTING THIS AS                        
C            ISYMD = 0 = DO NOTHING (DEFAULT)                           
C                    1 = PROJECT THE DENSITY BY SYMH, ONLY, ACCEPTING   
C                        THAT THE 1ST ITERATION WILL BE NON-VARIATIONAL 
C                    2 = GENERATE ORBITALS FROM THE PROJECTED DENSITY,  
C                        AND USE THEM TO MAKE A VARIATIONAL DENSITY,    
C                        WHICH IS THE PRESENT CODE                      
C                    3 = ALSO TRY TO DIAGONALIZE LITTLE H, IN JUST      
C                        OCCUPIED/VIRTUAL BLOCKS, TO TRY TO CLEAN       
C                        UP THE CRASILY-ENERGY-MIXED SITUATION.         
C                                                                       
...
C                                                                       
C     DENSITY TRANSFORMATION FROM AO BASIS TO Q BASIS IS V=S*Q          
C                                                                       
      CALL MRTRBR(XX(ISYMDEN1),L1,L1,L1,Q,L1,L0,V,L1)                   
C                                                                       
C     TRANSFORM DENSITY TO Q BASIS BY RHO(QBASIS) = (SQ)-DAGGER RHO (SQ)
C     DIAGONALIZE DENSITY (WHICH IS NOW IN Q BASIS), TO GET ORBITALS    
C                                                                       
      CALL TFTRI(XX(ISYMDEN1),DA,V,SCR,L0,L1,L1)                        
      CALL SYMDIA(XX(ISYMDEN1),V,SCR,XX(ISYMDEN2),IWRK,L0,L2,L1)        
C                                                                       
...
C     FINALLY, BACK-TRANSFORM VECTORS TO AO BASIS                       
C                                                                       
      DO I=1,INT(L0/2)                                                  
        DO J=1,L1                                                       
          DUMMY=V(J,I)                                                  
...
C                                                                       
C     REMAKE DENSITY FROM OUR WONDERFUL NEW ORBITALS.                   
C                                                                       
      NOCC=0                                                            
      DO I=1,L0                                                         
         IF(OCCNO(I).GT.1.0D-06) NOCC=NOCC+1                            
      ENDDO                                                             
      CALL DMTX(DA,V,OCCNO,NOCC,L1,L1)                                  
C                                                                       
C     PRINT OUT NEW ORBITALS                                            
C                                                                       
      IF (MASWRK .AND. NPASS.EQ.1) WRITE(IW,9020)                       
      IF (MASWRK .AND. NPASS.EQ.2) WRITE(IW,*) ' BETA '                 
      IF (MASWRK) WRITE(IW,9022) (I,LABMO(I),I=1,NOCC)                  
C     RETURN MEMORY                                                     
      CALL RETFM(NEED)                                                  
 9020 FORMAT(/1X,'SYMMETRIES FOR THE OCCUPIED "SYMDEN"-SYMMETRIZED',    
     *    ' ORBITALS FOLLOW.'/                                          
     *    1X,'DIAGONALIZATION TENDS TO SCRAMBLE ORDER OF ANY',          
     *       ' EQUIVALENTLY OCCUPIED ORBS,'/                            
     *    1X,'SO THESE ARE CERTAINLY NOT ORDERED BY ANY ENERGY',        
     *       ' CRITERION.'/                                             
     *    1X,'THIS SHOULDN''T AFFECT USING TOTAL DENSITY OF ALL MOS',   
     *       ' IN THE ENSUING SCF...')                                  
 9022 FORMAT(7(1X,I5,'=',A4))                                           
      RETURN                                                            
C-----------EXTRA CRISPY ORTHOG-------                                  
C---  END                                                               
C---  SUBROUTINE VERIFY_ONB(VEC,Q,S,SMO,WRK,L0,L1,L2)                   
C---  IMPLICIT DOUBLE PRECISION(A-H,O-Z)                                
C---  DIMENSION VEC(L1,*),Q(L1,*),S(L2),SMO(L2),WRK(L1)                 
C---  LOGICAL GOPARR,DSKWRK,MASWRK                                      
...
C                                                                       
C---  CALL DAREAD(IDAF,IODA,  S,L2   ,12,0)                             
C---  CALL DAREAD(IDAF,IODA,VEC,L1*L1,15,0)                             
C---  CALL DAREAD(IDAF,IODA,  Q,L1*L1,45,0)                             
C                                                                       
C        NOTE THAT TFTRI ZAPS SMALL VALUES,                             
C        SO AN ORTHOG MO SET PRINTS EXACTLY ZERO AS MAX OFF-DIAGONAL.   
C                                                                       
C---  CALL TFTRI(SMO,S,Q,WRK,L0,L1,L1)                                  
C---  DVAL= 0.0D+00                                                     
C---  OVAL= 0.0D+00                                                     
C---  IJ=0                                                              
C---  DO I=1,L0                                                         
...
C---        VAL = SMO(IJ)                                               
C---        IF(I.EQ.J) THEN                                             
C---           DVAL = MAX(DVAL,ABS(VAL-ONE))                            
C---        ELSE                                                        
C---           OVAL = MAX(OVAL,ABS(VAL))                                
C---        ENDIF                                                       
C---     ENDDO                                                          
C---  ENDDO                                                             
C---  IF(MASWRK) WRITE(6,2) 'Q',DVAL,OVAL                               
C                                                                       
C---  CALL TFTRI(SMO,S,VEC,WRK,L0,L1,L1)                                
C---  DVAL= 0.0D+00                                                     
C---  OVAL= 0.0D+00                                                     
C---  DVALSUM = 0.0D+00                                                 
C---  OVALSUM = 0.0D+00                                                 
...
C---        VAL = ABS(SMO(IJ))                                          
C---        IF(I.EQ.J) THEN                                             
C---           DVALSUM = DVALSUM + VAL                                  
C---           VAL=ABS(VAL-ONE)                                         
C---           IF(VAL.GT.DVAL) THEN                                     
C---              DVAL = VAL                                            
C---              IMAXD = I                                             
C---              JMAXD = J                                             
C---           END IF                                                   
C---        ELSE                                                        
C---           OVALSUM = OVALSUM + VAL                                  
C---           IF(VAL.GT.OVAL) THEN                                     
C---              OVAL = VAL                                            
C---              IMAXO = I                                             
C---              JMAXO = J                                             
C---           END IF                                                   
...
C---     ENDDO                                                          
C---  ENDDO                                                             
C---  IF(MASWRK) WRITE(6,3) 'VEC',DVAL,IMAXD,JMAXD,OVAL,IMAXO,JMAXO     
C---  DVALSUM = DVALSUM/L0                                              
C---  M2 = (L0*L0-L0)/2                                                 
C---  OVALSUM = OVALSUM/M2                                              
C---  IF(MASWRK) WRITE(6,4) 'VEC',DVALSUM,OVALSUM                       
C                                                                       
C---  RETURN                                                            
C---2 FORMAT(1X,'ORTHOG CHECK FOR ',A,' MATRIX:'/                       
C--- *       1X,'MAX.     DIAG DEVIATION FROM UNITY=',1P,E11.4/         
C--- *       1X,'MAX. OFF-DIAG DEVIATION FROM ZERO =',1P,E11.4)         
C---3 FORMAT(1X,'ORTHOG CHECK FOR ',A,' MATRIX:'/                       
C--- *       1X,'MAX.     DIAG DEVIATION FROM UNITY=',1P,E11.4,         
C--- *          0P,'  I=',I6,' J=',I6/                                  
C--- *       1X,'MAX. OFF-DIAG DEVIATION FROM ZERO =',1P,E11.4,         
C--- *          0P,'  I=',I6,' J=',I6)                                  
C---4 FORMAT(1X,'AVERAGES FOR ',A,' MATRIX:'/                           
C--- *       1X,'DIAGONAL=',F20.15,' OFF-DIAG=',F20.15)                 
C-----------EXTRA CRISPY ORTHOG-------                                  
      END                                                               
