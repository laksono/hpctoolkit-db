...
C 18 Apr 16 - HN,DGF - changes for FMO 5.2                              
C 22 Sep 15 - HL  - ADD FIXSOL TO QUANPOL                               
C 22 Oct 14 - DGF - changes for FMO 5.1                                 
C  1 Aug 14 - MWS - paper over EFP2's EXR spherical harmonic problem.   
C xx Oct 13 - AP  - add DEFESR terms in SVP to create new CMIRS1.0 model
...
C*MODULE RHFUHF  *DECK HSTARLC                                          
      ...
      USE camdft, ONLY: BETAC => cam_beta, CAMFLAG                      
...
C                                                                       
  220 CONTINUE                                                          
      L2 = (NUM*NUM+NUM)/2                                              
      CALL XSCAL(L2,HALF,FA,1)                                          
      CALL XSCAL(L2,HALF,FB,1)                                          
      II = 0                                                            
      DO 250 I=1,NUM                                                    
         II = II + I                                                    
         FA(II) = FA(II)+FA(II)                                         
...
C                                                                       
  ...
  510 CONTINUE                                                          
      DO 520 I = 1,NUMTRI                                               
         ...
         DB(I) = (DUMA - DUMB)                                          
  520 CONTINUE                                                          
      CALL XSCAL(NUMTRI,HALF,DA,1)                                      
      CALL XSCAL(NUMTRI,HALF,DB,1)                                      
      CALL XSCAL(NUMTRI,HALF,FA,1)                                      
      CALL XSCAL(NUMTRI,HALF,FB,1)                                      
      CALL SEQREW(IS)                                                   
      RETURN                                                            
C     ******                                                            
  610 CONTINUE                                                          
...
C                                                                       
      CALL XSCAL(L2,HALF,JCORE,1)                                       
      CALL XSCAL(L2,HALF,JVAL ,1)                                       
      CALL XSCAL(L2,HALF,KCORE,1)                                       
      CALL XSCAL(L2,HALF,KVAL ,1)                                       
      II = 0                                                            
      DO 250 I=1,L1                                                     
         II = II + I                                                    
...
C>    -Modified OVLSEL arguments.                                       
C>                                                                      
C>    @date Aug, 2014 - Jimmy Kromann                                   
C>    - Added -D3H+ correction energies for PM6                         
C>                                                                      
C>    @date Jul, 2015 - Peng Xu                                         
C>    - Added code to call EFP-AI EXREP energy                          
C>                                                                      
C>    @date March, 2016 - Soumya Ghosh                                  
C>    - Added code related to Reorganization energy calculations:       
C>      1) 2 common blocks 2) final energy definition 3) Read arrays to 
C>      print surface charges to the punch file                         
C>                                                                      
...
C>    - adding localization options for QM-EFP2 dispersion              
C>    - implement parallel QM-EFP2 exchange repulsion                   
C>       force FDIFF=.F. when DIRSCF=.T. and use tigher density         
C>       convergence where DENTOL = CONVHF                              
C>    - calculate QM-EFP2 EXREP every SCF iterations and no longer      
C>      add EEXCH to the total energy                                   
C>                                                                      
C>    @date October 2019, Peng Xu and Tosaporn Sattasathuchana          
C>    - adding new driver subroutine for QMEFP2 dispersion              
C>           (EFAIDISPDR)                                               
C>    - QMEFP2 is now interfaced with MP2 and Coupled Cluster           
C>                                                                      
      SUBROUTINE RHFCL                                                  
c                                                                       
      USE metaGGA, ONLY: NEEDTAU, PRTTAU => printtau                    
      USE camdft, ONLY: ALPHAC => cam_alpha, BETAC => cam_beta,         
     *  CAMMU => cam_mu, CAMVWN => cam_vwn5, CAMLYP => cam_lyp,         
     *  CAMFLAG                                                         
      USE dhdft, ONLY: CHF, CMP2, C2S, C2T, DHFUNC                      
      USE lrcdft, ONLY: LCFLAG, EMU, EMU2, LRFILE                       
      USE MX_LIMITS,ONLY:MXFRG,MXDFG,MXDPPT,mxsh,mxgtot,mxgsh,mxg2,     
     *     mxatm,mxao,mxrt,mxgrid                                       
      USE comm_EFPIO                                                    
      USE comm_FRGINF                                                   
      USE comm_EFPFMO                                                   
      USE EFAIDIS, ONLY : IQMDISP, DISPENRG, MMCORE                     
      USE hp_hbcorr, ONLY: LMPCHBOND, HBOND_ENERGY                      
      USE EFAIEXREP, ONLY : EEXCH                                       
      use semiempirical_corrections, only: lgcp, lsrb                   
      USE modmcpdft, ONLY: MCPDFTRUN,EPDFT                              
     ...
     *        FMOEX,FMOPL,LRINT,LCFLAGS,LRINTS,                         
     *        SG1,DC,DCCHG,DCABC,locopt,wasgddi,mlgddi,                 
     *        UHFCHK,inmem15,qmcout,scfesp                              
C                                                                       
...
CJCK                                                                    
      DOUBLE PRECISION hb_param(2)                                      
CJCK                                                                    
C     LIBCCHEM information                                              
      LOGICAL GMS_CCHEM_HF,GMS_CCHEM_RI,DENFIT                          
...
C                                                                       
      ...
      END IF                                                            
C                                                                       
      if(GMS_CCHEM_RI() .and. DENFIT)THEN                               
         LF_ACCUM = LAST                                                
         LAST = LF_ACCUM +L2                                            
      endif                                                             
C                                                                       
...
c        There is no known limit to ITSO (the number of                 
c        SOSCF vectors), assume MAXIT.                                  
      ELSE                                                              
         LGRAD  = LAST                                                  
         LHESS  = LAST                                                  
         LPGRAD = LAST                                                  
...
C                                                                       
      ...
      IF(NDFTFG.EQ.1) THEN                                              
         ...
         IF(MCPDFTRUN) ISWOFF=0                                         
         IF(NFG.NE.0.AND.IPIEDA.NE.0) ISWOFF=0                          
c        switching grid and turning off DFT will mess up EX component.  
      END IF                                                            
      HFSSAV=DFTTYP(3)                                                  
      NDFTSV=NDFTFG                                                     
      IDFTSWITCH=0                                                      
...
C                                                                       
      IF(DIRSCF) THEN                                                   
         DENTOL = TWO * CONVHF                                          
C- FOR QM/EFP2 USE TIGHTER DENTOL                                       
         IF(LVLEFP.EQ.2) DENTOL=CONVHF                                  
         ENGTOL = 1.0D-08                                               
         DIITOL = 1.0D-06                                               
      ELSE                                                              
         DENTOL = CONVHF                                                
...
C        IF(SW0.LT.CONVHF.AND.SW0.NE.0) THEN                            
         IF(SW0.EQ.0.0D+00) THEN                                        
            IF (MASWRK) WRITE(IW,'(5X,A)') 'DFT GRID SWITCHING DISABLED'
         ELSE IF(SW0.LT.CONVHF) THEN                                    
...
C                                                                       
     ...
     *   .AND. (NEVALS.GT.0.AND.NFG.EQ.0)) THEN                         
         GRMS = XDOT(3*NAT,EGRAD,1,EGRAD,1)                             
...
C                                                                       
     ...
     *                     NHCO(3,I),NHCO(4,I),ANGLE,SIGNA)             
         ...
         ENDDO                                                          
CJCK         EHNCO = EHNCO/627.52791D+00                                
         EHNCO = EHNCO/FPC(13)                                          
      END IF                                                            
C                                                                       
...
C                                                                       
      ...
      IF(IPCMIT.EQ.0  .AND.  (IPCM.EQ.1 .OR. IPCFP)) THEN               
         IF (IEF.EQ.9.OR.IEF.EQ.11.OR.IEF.EQ.13.OR.IEF.EQ.14) GOTO 635  
         NTS3 = NTS*NTS                                                 
         CALL SEQREW(NFT26)                                             
...
C                                                                       
      ...
      IF(DIRSCF.AND.ABINIT) THEN                                        
         SCHWRZ = ISCHWZ.GT.0                                           
c---PX: force FDIFF=F when DIRSCF=T for QM-EFP2                         
         IF(LVLEFP .EQ. 2) THEN                                         
            FDIFF=.FALSE.                                               
            IF(MASWRK) WRITE(IW,9029) FDIFF                             
         END IF                                                         
...
C          LMOEDA DOES NOT LIKE SWITCHING GRID                          
         IF(IPCM.EQ.1 .OR. RUNTYP.EQ.LMOEDA .OR.                        
     *      NFG.NE.0.AND.IPIEDA.NE.0) THEN                              
            NRAD0 = NRAD                                                
            NPHI0 = NPHI                                                
            NTHE0 = NTHE                                                
            NLEB0(1) = NLEB(1)                                          
         END IF                                                         
         IF(SWDFT.EQ.0.0D+00) THEN                                      
            GRIDTYP=FINE                                                
         ELSE IF(SG1  .OR.  JANS.GT.0) THEN                             
            GRIDTYP=FINE                                                
         ELSE IF(MCPDFTRUN) THEN                                        
            GRIDTYP=FINE                                                
         ELSE                                                           
            IF (NLEB(1).NE.0) THEN                                      
...
C                                                                       
      IF(ABINIT) THEN                                                   
         IF(DIRSCF) THEN                                                
            ...
            ELSE                                                        
               CALL XCOPY(L2,X(LD),1,X(LWRK1),1)                        
            END IF                                                      
            IF(SCHWRZ) THEN                                             
               DUMMY = 0.0D+00                                          
...
C                                                                       
                  call stopwa(4,1)                                      
               ELSE                                                     
                   IF (GMS_CCHEM_HF() .AND. .NOT.DENFIT) THEN           
C                                                                       
                        CALL GMS_CCHEM_HF_FOCK(X(LWRK1), X(LFAO),       
     *                       X(LXINTS), CUTOFF)                         
C                                                                       
                   ELSE IF (GMS_CCHEM_RI() .AND. DENFIT) THEN           
C                                                                       
C     GET THE CURRENT EIGENVECTORS                                      
                        IF(ITER .EQ. 1)THEN                             
                           CALL DAREAD(IDAF,IODA,X(LVEC),L3,15,0)       
                           CALL VCLR(X(LF_ACCUM),1,L2)                  
                        ENDIF                                           
                                                                        
C     BUILD 2E FOCK MATRIX WITH DENISTY FITTING                         
                        CALL GMS_CCHEM_DF_RHF(                          
     $                       X(LWRK1),X(LWRK1),                         
     $                       X(LFAO),X(LFAO),                           
     $                       X(LXINTS), CUTOFF, X(LVEC))                
                                                                        
C     COMPUTE THE CHANGE IN THE 2E FOCK MATRIX                          
                        IF(ITER .GT. 1)THEN                             
                           CALL XAXPY(L2, -1.0D+00, X(LF_ACCUM), 1,     
     $                          X(LFAO), 1)                             
                        ENDIF                                           
                                                                        
C     ACCUMUALTE THE 2E FOCK MATRIX                                     
                        CALL XAXPY(L2, 1.0D+00, X(LFAO), 1,             
     $                       X(LF_ACCUM), 1)                            
C                                                                       
                   ELSE                                                 
                     CALL TWOEI(RHF,DIRSCF,.FALSE.,.FALSE.,.FALSE.,     
...
C                                                                       
           ...
           EDFT(2)=TOTELE                                               
C                                                                       
           IF (NEEDTAU.OR.PRTTAU) THEN                                  
             IF(GOPARR) CALL DDI_GSUMF(2317,TOTKIN,1)                   
             IF (PRTTAU) WRITE(IW,9326) TOTKIN                          
           END IF                                                       
C                                                                       
...
C                                                                       
     ...
     *              ITER,EN,X(IGRDDFT),X(LVEC),X(LVEC),VXCEXCDIFF)      
      END IF                                                            
C                                                                       
...
C                                                                       
         ...
         ELSE                                                           
            CALL XCOPY(L2,X(LD),1,X(LWRK2),1)                           
         END IF                                                         
     ...
     *               X(LXYZRE+MXSP*3),X(LISPHE),X(LLIST),X(LEPSPCM))    
      ...
      IF(NFMOPCM.NE.0) THEN                                             
        CALL DAREAD(IDAF,IODA,X(LWRK2),L2,313,0)                        
...
C                                                                       
         IF(NBUFMO.GT.0) CALL XCOPY(L2,X(LWRK1),1,X(LD),1)              
         call stopwa(7,1)                                               
      END IF                                                            
C                                                                       
...
C                                                                       
      ...
      IF(IEFP.EQ.1) THEN                                                
         CALL DAREAD(IDAF,IODA,X(LH1),LL2,87,0)                         
C                                                                       
C           To whom it may concern.  The old EXR code from Dan Kemp     
C           lacked the use of L0 in the transformations before and      
C           after computing the EFHFDR term.  Zeroing the high part     
C           of the matrix seems prudent, in view of the lack of an L0   
C           value passed to the actual calculation.  It was noted       
C           that the higher part of the H1 matrix remains zero, so      
C           the real computation may be internally aware of spherical   
C           harmonics?  Note also that the programming of Dan Kemp      
C           is scheduled for a massive update by Peng Xu someday.       
C           This remark can be removed at that time.  (MWS-7/2014)      
         IF(LVLEFP.EQ.2) THEN                                           
            IF(ITER.EQ.1) THEN                                          
               CALL DIPINT(ZERO,ZERO,ZERO,.FALSE.)                      
            ELSE                                                        
               CALL TFTRI(X(LHMO),X(LH1),X(LVEC),X(LWRK1),L0,L1,L1)     
               IF(L0.LT.L1) THEN                                        
                   L2SPH = (L0*L0+L0)/2                                 
                   CALL VCLR(X(LHMO+L2SPH),1,L2-L2SPH)                  
               END IF                                                   
               CALL EFHFDR(X(LHMO),X(LVMI),X(LVMITR),EEXCH,L1,L2,CVGED, 
     *              0)                                                  
...
C   WE WANT THE FULL DENSITY BACK AFTER EFPCMP IS DONE                  
         IF(NBUFMO.GT.0) THEN                                           
            CALL XCOPY(L2,X(LWRK1),1,X(LD),1)                           
         END IF                                                         
      END IF                                                            
c                                                                       
c     FMO ESP                                                           
c                                                                       
      if(scfesp) CALL FMOESPF(DIRSCF.AND.fdiff,iter,L1,L2,              
     *                        X(LH1),x(lfao),x(LWRK2))                  
C                                                                       
C                      ----- COSMO -----                                
...
C                                                                       
     ...
     *                      X(LFFQRXN2),                                
     *                      X(LFFCMAT1),L2,L1,0,ISPHSOL,X(LFFNONLSTQ))  
         ELSE                                                           
...
c       write(6,*) 'Fock before FRFOCK1'                                
c       call prtril(X(LFAO),l1)                                         
...
C                                                                       
      IF (IREORG.EQ.1) THEN                                             
         ETOT=ETOT+HALF*PX                                              
         EPLXX=ETOT-HALF*PX-POLXX                                       
      ELSEIF (IREORG.EQ.0) THEN                                         
         IF(IPCMIT.EQ.0) THEN                                           
...
C        WRITE(6,*) 'WWWEX',E0SCF(1)                                    
         IF(NORBPROJ.NE.0) THEN                                         
            if(rflmo(1).ne.0) then                                      
              CALL DAREAD(IDAF,IODA,X(LVEC),LL3,318,0)                  
              CALL DAREAD(IDAF,IODA,X(LWRK3),L2,317,0)                  
              CALL TFTRI(X(LWRK1),X(LWRK3),X(LVEC),X(LWRK),L0,L1,L1)    
              NORFRZs=NORFRZ                                            
              NORFRZ=numfrz                                             
              CALL FRFOCK1(X(LWRK1),L1)                                 
              NORFRZ=NORFRZs                                            
              IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LS),LL2,12,0)          
              CALL TFTRIB(X(LWRK3),X(LWRK1),X(LS),X(LVEC),X(LWRK2),     
     *                    X(LIWRK),L0,L1,L2,L3)                         
              EHF1Cp= TRACEP(X(LD),X(LWRK3),L1)                         
              EPROJ=(EHF1-EHF1Cp)/two                                   
c             (Tr(D*F_SCF)-Tr(D*F_f))/2,                                
c             where F_SCF and F_f are the Fock matrices used in SCF     
c             (with the projection) and that without it, respectively.  
            else                                                        
...
c           WRITE(6,*) 'WWWEPROJ',EPROJ,E0SCF(1)                        
         ENDIF                                                          
         if(NDFTFG.EQ.1) E0SCF(1)=E0SCF(1)-ECORL                        
c        if(NDFTFG.EQ.1) write(6,*) 'subtracted',ECORL                  
      ENDIF                                                             
...
C                                                                       
     ...
     *                  NPR,L0,L1,NA,NA,ORBGRD)                         
            CALL XCOPY(NPR,X(LGRAD),1,X(LPGRAD),1)                      
            GO TO 800                                                   
         END IF                                                         
      END IF                                                            
C                                                                       
...
C                                                                       
      IF(DOAFO) CALL DAWRIT(IDAF,IODA,X(LFAO),L2,314,0)                 
c                                                                       
      IF(ABINIT) THEN                                                   
         IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LQ),LL3,45,0)               
...
C                                                                       
      IF((RSTRCT.OR.MFRZ).AND..NOT.DOAFO) THEN                          
         IVREC=15                                                       
c        IF(NFG.NE.0.AND.ITER.EQ.1) IVREC=318                           
...
c         IF(NRAD*NANGPT(1).LT.NRAD0*NANGPT0(1)  .AND.                  
c     *       .NOT.(SG1.OR.JANS.GT.0)) THEN                             
         IF(GRIDTYP.NE.FINE) THEN                                       
...
C---  IF(DODIIS  .AND.  SWDIIS.GT.0.0D+00  .AND.  DIFF.LT.SWDIIS) THEN  
     ...
     *   (IDFTSWITCH.EQ.IDFTSWITCH_SAVE.OR.DIRSCF.AND..NOT.FDIFF)) THEN 
        ...
        IF(MASWRK.and.iand(modio,16).eq.0) WRITE(IW,9380)               
        IDFTSWITCH=4                                                    
      END IF                                                            
C                                                                       
...
C                                                                       
      IF(IDFTSWITCH.NE.0) THEN                                          
        ...
        IDFTSWITCH=0                                                    
      END IF                                                            
      if(scfesp) call FMOCNV(l1,l2,l3,iter,CVGED,nconv)                 
C                                                                       
...
C                                                                       
      IF(NFG.NE.0.AND.NDFTFG.EQ.1) THEN                                 
c       IF (NRAD*NANGPT(1).LT.NRAD0*NANGPT0(1)  .AND.                   
c     *       .NOT.(SG1.OR.JANS.GT.0)) THEN                             
       IF(GRIDTYP.NE.FINE) THEN                                         
        CALL SWGRID                                                     
...
C                                                                       
      IF(LVLEFP.EQ.2) THEN                                              
         CALL EFHFDR(X(LHMO),X(LVMI),X(LVMITR),EEXCH,L1,L2,CVGED,1)     
      END IF                                                            
C NOW QM/EFP2 EXREP IS CALCULATED EVERY SCF ITERATIONS.                 
C THE EEXCH IS ALREADY INCLUDED IN ETOT, SO NO LONGER ADDING            
C EEXCH TO ETOT ANYMORE.                                                
C     ---- ADD QM-EFP EXCHANGE REPULSION                                
C      ETOT = ETOT + EEXCH                                              
C                                                                       
C                                                                       
...
C                                                                       
      ...
      END IF                                                            
C                                                                       
C     ----- SEMI-EMPIRICAL HYDROGEN CORRECTION (KORTH DH+)              
C                                                                       
CJCK                                                                    
      IF(MPCTYP.NE.NONE .AND. LMPCHBOND) THEN                           
C subroutine hbond_energy(natoms, geo, labels, param, hb_energy)        
         hb_param(1) = -0.11d+00                                        
         hb_param(2) = -0.12d+00                                        
         EHB = 0.0D+00                                                  
         CALL HBOND_ENERGY(NAT, C, IAN, hb_param, EHB)                  
         ETOT = ETOT + EHB                                              
      END IF                                                            
CJCK                                                                    
C                                                                       
...
CJCK               DELTAH = ETOT*627.52791D+00 + ATHEAT                 
CJCK                             627.5096                               
CJCK                                                                    
               IF(LMPCHBOND.OR.DC) THEN                                 
CJCK               Print Hydrogen-bond and dispersion correction        
CJCK               contribution terms to the energy                     
                WRITE(IW,9176)                                          
               ENDIF                                                    
               IF(LMPCHBOND) THEN                                       
                WRITE(IW,9177) EHB                                      
...
CCC         QT=   QSE(I) +  QSN(I) + Q_FS(I) + Q_IND(I)                 
         ...
         IF(MASPRT) THEN                                                
            IF(IPCMIT.EQ.0) THEN                                        
               ...
               IF (IREORG.EQ.1 .AND. IPRNTF.EQ.1) THEN                  
                  CALL VALFM(LOADFM)                                    
                  LPLTOT = LOADFM + 1                                   
                  LVNTOT = LPLTOT + NTS                                 
                  LQETOT = LVNTOT + NTS                                 
...
C                                                                       
                  ...
                  CALL SEQADV(NFT26)                                    
                  CALL SEQADV(NFT26)                                    
                  CALL SQREAD(NFT26,X(LPLTOT),NTS)                      
                  CALL SQREAD(NFT26,X(LVNTOT),NTS)                      
                  CALL SQREAD(NFT26,X(LQETOT),NTS)                      
                  CALL SQREAD(NFT26,X(LPLFST),NTS)                      
                  CALL SQREAD(NFT26,X(LVNFST),NTS)                      
                  CALL SQREAD(NFT26,X(LQEFST),NTS)                      
                  CALL SQREAD(NFT26,X(LPLSLW),NTS)                      
                  CALL SQREAD(NFT26,X(LVNSLW),NTS)                      
                  CALL SQREAD(NFT26,X(LQESLW),NTS)                      
                  CALL SQREAD(NFT26,X(LXTLAM),NTS)                      
                  CALL SQREAD(NFT26,X(LYTLAM),NTS)                      
                  CALL SQREAD(NFT26,X(LZTLAM),NTS)                      
                  CALL SEQREW(NFT26)                                    
                                                                        
C                                                                       
                  CALL RORGPE(NTS,LAMVAL,X(LPLTOT),X(LVNTOT),X(LQETOT), 
     *                                   X(LPLFST),X(LVNFST),X(LQEFST), 
     *                                   X(LPLSLW),X(LVNSLW),X(LQESLW), 
     *                                   X(LXTLAM),X(LYTLAM),X(LZTLAM)) 
C                                                                       
                  CALL RETFM(NEEDR1)                                    
C                                                                       
               ELSEIF (IREORG.EQ.1 .AND. IREADF.EQ.1) THEN              
                  CALL VALFM(LOADFM)                                    
                  LPLNQ = LOADFM + 1                                    
                  LVNNQ = LPLNQ + NTS                                   
                  LQENQ = LVNNQ + NTS                                   
...
C                                                                       
                  ...
                  CALL SEQADV(NFT26)                                    
                  CALL SEQADV(NFT26)                                    
                  CALL SQREAD(NFT26,X(LPLNQ),NTS)                       
                  CALL SQREAD(NFT26,X(LVNNQ),NTS)                       
                  CALL SQREAD(NFT26,X(LQENQ),NTS)                       
                  CALL SQREAD(NFT26,X(LPLNQF),NTS)                      
                  CALL SQREAD(NFT26,X(LVNNQF),NTS)                      
                  CALL SQREAD(NFT26,X(LQENQF),NTS)                      
                  CALL SQREAD(NFT26,X(LPLNQS),NTS)                      
                  CALL SQREAD(NFT26,X(LVNNQS),NTS)                      
                  CALL SQREAD(NFT26,X(LQENQS),NTS)                      
                  CALL SQREAD(NFT26,X(LXTLAM),NTS)                      
                  CALL SQREAD(NFT26,X(LYTLAM),NTS)                      
                  CALL SQREAD(NFT26,X(LZTLAM),NTS)                      
                  CALL SEQREW(NFT26)                                    
C                                                                       
                  CALL RORGPE(NTS,LAMVAL,X(LPLNQ),X(LVNNQ),X(LQENQ),    
     *                                   X(LPLNQF),X(LVNNQF),X(LQENQF), 
     *                                   X(LPLNQS),X(LVNNQS),X(LQENQS), 
     *                                   X(LXTLAM),X(LYTLAM),X(LZTLAM)) 
C                                                                       
                  CALL RETFM(NEEDR1)                                    
               ENDIF                                                    
C                                                                       
      ...
      END IF                                                            
C                                                                       
C         SOLVATION OUTPUT FOR NEW CMIRS MODEL ~ SVP + DEFESR           
C                                                                       
      IF(ISVP.EQ.1  .AND.  MASPRT) THEN                                 
         CALL DAREAD(IDAF,IODA,X(LVEC),L3,15,0)                         
         CALL TRPOSE(X(LVEC),X(LWRK1),L1,L1,1)                          
         CALL SVPOUT(X(IGRDDFT),X(LVEC),X(LVEC),L1,L2,                  
     *               X(LD),X(LD),ETOT,ECHIP,NPRINT,'RHFCL ')            
         CALL TRPOSE(X(LVEC),X(LWRK1),L1,L1,1)                          
      END IF                                                            
C                                                                       
...
C     SUPRESS PRINT OUT OF EIGENVECTORS AND EIGENVALUES FOR MCPDFT RUNS 
         IF(MCPDFTRUN.NEQV..TRUE.) THEN                                 
           CALL PRSELMO(X(LVEC),X(LEIG),X(LWRK),L0,L1,L1)               
         ENDIF                                                          
      END IF                                                            
      qmcout=nfg.ne.0.and.iand(nfmopal,8).ne.0                          
      IF(SOME.and.qmcout.and.idoprop.ne.0) then                         
        nftqmc=ip                                                       
        CALL PREVSQMC(nftqmc,X(LVEC),X(LEIG),X(LWRK),L0,L1,L1,1)        
      ENDIF                                                             
                                                                        
      IF(MCPDFTRUN.EQV..TRUE.) PRDENS=.FALSE.                           
      IF(PRDENS) THEN                                                   
         WRITE(IW,9190)                                                 
         CALL PRTRIL(X(LD),L1)                                          
      END IF                                                            
...
C                                                                       
     ...
     *   WRITE(IW,FMT='('' ...... END OF RHF CALCULATION ......'')')    
      if(nfg.eq.0.or.iand(modio,256).eq.0) CALL TIMIT(1)                
      RETURN                                                            
C                                                                       
...
C                                                                       
 ...
 9029 FORMAT(/1X,'QM/EFP2 CALCULATION, FDIFF=',L1,' FOR DIRSCF=T')      
 ...
 9176 FORMAT(/1X,'SEMI-EMPIRICAL ENERGY CONTRIBUTIONS:')                
 9177 FORMAT(1X,'HYDROGEN BOND                                  =',     
     *           F20.10)                                                
 9510 FORMAT(1X,'GRIMME''S DISPERSION ENERGY                     =',    
     *           F20.10)                                                
 9520 FORMAT(1X,'GRIMME''S CORRECTION ENERGY                     =',    
     *           F20.10)                                                
 9530 FORMAT(1X,'ENERGY WITHOUT GRIMME''S CORRECTION ENERGY      =',    
     *           F20.10)                                                
...
C>    @date 9-4-2-18 Luke Roskop                                        
C>      -updated for libcchem Density Fitting                           
C>      -change spin calculaton for SCFTYP=ROHF                         
C>                                                                      
...
C                                                                       
     ...
     *        IPCFP,LRINT,LCFLAGS,LRINTS,SG1,                           
     *        ISGDDI,PAROUT,INITGDDI,DC,DCCHG,DCABC,                    
     *        locopt,reset,wasgddi,mlgddi,DOAFO                         
      CHARACTER*11 METHOD                                               
C                                                                       
  ...
  310 CONTINUE                                                          
      ...
      IF(ABINIT) THEN                                                   
         IF(DIRSCF) THEN                                                
            ...
            ELSE                                                        
               CALL XCOPY(L2,X(LDA),1,X(LWRK1),1)                       
               CALL XCOPY(L2,X(LDB),1,X(LWRK2),1)                       
            END IF                                                      
C                                                                       
            IF(SCHWRZ) THEN                                             
               DUMMY = 0.0D+00                                          
               ...
               ELSE                                                     
                 IF (GMS_CCHEM_RI().AND.DENFIT) THEN                    
                      IF(ITER .EQ. 1)THEN                               
                         ...
                         CALL VCLR(X(LFA_ACCUM),1,L2)                   
                         CALL VCLR(X(LFB_ACCUM),1,L2)                   
                      ENDIF                                             
C     BUILD 2E FOCK MATRIX WITH DENISTY FITTING                         
                      CALL GMS_CCHEM_DF_RHF(                            
     $                     X(LWRK1),X(LWRK2),                           
     $                     X(LFA),X(LFB),                               
     $                     X(LXINTS), CUTOFF, X(LVA) )                  
C     COMPUTE THE CHANGE IN THE 2E FOCK MATRIX                          
                      IF(ITER .GT. 1)THEN                               
                         CALL XAXPY(L2, -1.0D+00, X(LFA_ACCUM), 1,      
     $                        X(LFA), 1)                                
                         CALL XAXPY(L2, -1.0D+00, X(LFB_ACCUM), 1,      
     $                        X(LFB), 1)                                
                      ENDIF                                             
C     ACCUMUALTE THE 2E FOCK MATRIX                                     
                      CALL XAXPY(L2, 1.0D+00, X(LFA), 1,                
     $                     X(LFA_ACCUM), 1)                             
                      CALL XAXPY(L2, 1.0D+00, X(LFB), 1,                
     $                     X(LFB_ACCUM), 1)                             
                 ELSE                                                   
                    CALL TWOEI(SCFTYP,DIRSCF,.FALSE.,.FALSE.,.FALSE.,   
     *                   INTTYP,SCHWRZ,NINT,NSCHWZ,L1,L2,               
     ...
     *                   X(LDSH),DUMMY,DUMMY,1)                         
               ...
               CUTOFF = CUTSV                                           
            END IF                                                      
            CALL XSCAL(L2,HALF,X(LFA),1)                                
            CALL XSCAL(L2,HALF,X(LFB),1)                                
            II=LFA-1                                                    
            JJ=LFB-1                                                    
            DO 210 I=1,L1                                               
               II = II+I                                                
...
C               ATTENUATED INTEGRALS FOR CAM-B3LYP DFT                  
     ...
     *                       X(LXK),NINTMX,IA)                          
               CALL XSCAL(L2,HALF,X(LAFA),1)                            
               CALL XSCAL(L2,HALF,X(LAFB),1)                            
               II=LAFA-1                                                
               JJ=LAFB-1                                                
               DO I=1,L1                                                
                  II = II+I                                             
...
C                                                                       
     ...
     *              ICNT,EHFCL,ITER,EN,X(IGRDDFT),X(LVA),X(LVB),        
     *              VXCEXCDIFF)                                         
      END IF                                                            
C                                                                       
C     * * * * * SOLVATION MODEL CORRECTIONS TO 1E- TERM * * * * *       
...
C                                                                       
         IF(NBUFMO.GT.0) THEN                                           
            ...
            END IF                                                      
         ELSE                                                           
            CALL XCOPY(L2,X(LWRK1),1,X(LWRK2),1)                        
         END IF                                                         
C                                                                       
...
CXXX  Only for FMO                                                      
         IF(MFRZ .AND. ITER .GT. 1) THEN                                
           CALL DAREAD(IDAF,IODA,X(LWRK1),L2*2,317,0)                   
           CALL VADD(X(LWRK1   ),1,X(LFA),1,X(LWRK1),1,L2)              
           CALL VADD(X(LWRK1+L2),1,X(LFB),1,X(LWRK1+L2),1,L2)           
           CALL DAWRIT(IDAF,IODA,X(LWRK1),L2*2,317,0)                   
         END IF                                                         
...
C           WRITE(IW,*)"ADDING DIFFERENCE HNEW-HOLD TO F"               
         ...
         ENDIF                                                          
         IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LH1),L2,11,0)               
         IF(MFRZ .AND. ITER .EQ. 1) THEN                                
           CALL XCOPY(L2,X(LFA),1,X(LWRK1),1)                           
           CALL XCOPY(L2,X(LFB),1,X(LWRK1+L2),1)                        
           CALL DAWRIT(IDAF,IODA,X(LWRK1),L2*2,317,0)                   
         END IF                                                         
      ELSE                                                              
         IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LH1),L2,11,0)               
...
C                                                                       
      EHF1 = TRACEP(X(LDB),X(LFB),L1) + TRACEP(X(LDA),X(LFA),L1)        
C                                                                       
...
C        WRITE(6,*) 'WWWEX',E0SCF(1)                                    
         IF(NORBPROJ.NE.0) THEN                                         
           if(rflmo(1).ne.0) then                                       
             CALL DAREAD(IDAF,IODA,X(LVA),L3,318,0)                     
             CALL DAREAD(IDAF,IODA,X(LWRK1),L2*2,317,0)                 
C           For alpha                                                   
             CALL TFTRI(X(LWRK3),X(LWRK1),X(LVA),X(LWRK),L0,L1,L1)      
             NORFRZs = NORFRZ                                           
             NORFRZ  = numfrz                                           
             CALL FRFOCK1(X(LWRK3),L1)                                  
             NORFRZ  = NORFRZs                                          
             IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LS),L2,12,0)            
             CALL TFTRIB(X(LWRK1),X(LWRK3),X(LS),X(LVA),X(LWRK2),       
     *                   X(LIWRK),L0,L1,L2,L3)                          
C           For beta                                                    
             CALL TFTRI(X(LWRK3),X(LWRK1+L2),X(LVA),X(LWRK),L0,L1,L1)   
             NORFRZs = NORFRZ                                           
             NORFRZ  = numfrz                                           
             CALL FRFOCK1(X(LWRK3),L1)                                  
             NORFRZ  = NORFRZs                                          
             IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LS),L2,12,0)            
             CALL TFTRIB(X(LWRK1+L2),X(LWRK3),X(LS),X(LVA),X(LWRK2),    
     *                   X(LIWRK),L0,L1,L2,L3)                          
C                                                                       
             EHF1Cp = TRACEP(X(LDA),X(LWRK1),L1)                        
     *              + TRACEP(X(LDB),X(LWRK1+L2),L1)                     
             EPROJ=(EHF1-EHF1Cp)/two                                    
c            (Tr(D*F_SCF)-Tr(D*F_f))/2,                                 
c            where F_SCF and F_f are the Fock matrices used in SCF      
c            (with the projection) and that without it, respectively.   
...
C                                                                       
      ...
      END IF                                                            
C XXX                                                                   
      IF(MFRZ.and.doafo) THEN                                           
C       Projection MO                                                   
        CALL DAREAD(IDAF,IODA,X(LVA),L3,318,0)                          
C       For Alpha                                                       
        CALL TFTRI(X(LWRK1),X(LFA),X(LVA),X(LWRK),L0,L1,L1)             
C                                                                       
c       WRITE(IW,*) 'before projection'                                 
c       CALL PRTRIL(X(lwrk1),L1)                                        
C                                                                       
        CALL FRFOCK1(X(LWRK1),L1)                                       
        IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LS),L2,12,0)                 
        CALL TFTRIB(X(LFA),X(LWRK1),X(LS),X(LVA),X(LWRK2),              
     *              X(LIWRK),L0,L1,L2,L3)                               
C       For Beta                                                        
        CALL TFTRI(X(LWRK1),X(LFB),X(LVA),X(LWRK),L0,L1,L1)             
        CALL FRFOCK1(X(LWRK1),L1)                                       
        IF(MINMEM) CALL DAREAD(IDAF,IODA,X(LS),L2,12,0)                 
        CALL TFTRIB(X(LFB),X(LWRK1),X(LS),X(LVA),X(LWRK2),              
     *              X(LIWRK),L0,L1,L2,L3)                               
C                                                                       
        IF(.NOT.FDIFF  .AND.  NDFTFG.NE.1) THEN                         
          CALL DAWRIT(IDAF,IODA,X(LFA),L2,14,0)                         
          CALL DAWRIT(IDAF,IODA,X(LFB),L2,18,0)                         
        END IF                                                          
                                                                        
        EHF1C = TRACEP(X(LDA),X(LFA),L1)                                
     *        + TRACEP(X(LDB),X(LFB),L1)                                
        IF(NDFTFG.EQ.1.AND.ABINIT.AND.DIRSCF.AND.FDIFF) THEN            
          EHF1C=EHF1C-TRACEP(X(LDA),X(LFXCA),L1)                        
          EHF1C=EHF1C-TRACEP(X(LDB),X(LFXCB),L1)                        
        END IF                                                          
C       WRITE(6,*) 'E CORRECTED',EHF+(EHF1C-EHF1)/TWO+EN                
        IF(NFG.NE.0) THEN                                               
          ETOT=ETOT  + (EHF1C-EHF1)/TWO                                 
          EHF1=EHF1C                                                    
        ENDIF                                                           
        IF(NFG.NE.0.AND.ITER.GT.1)                                      
     *    CALL DAREAD(IDAF,IODA,X(LVA),L3,15,0)                         
...
C                                                                       
      IF(SCFTYP.EQ.UHF  .AND.  ICUHF.EQ.1) THEN                         
         ...
         call XCOPY(l2,x(lda),1,x(lwrk5),1)                             
         call XCOPY(l2,x(ldb),1,x(lwrk6),1)                             
...
C                                                                       
     ...
     *                  L0,L1,ORBGRB)                                   
         ...
         IF(SCFTYP.EQ.UHF) THEN                                         
            CALL XCOPY(NPRA,X(LGRADA),1,X(LGRADU)     ,1)               
            CALL XCOPY(NPRB,X(LGRADB),1,X(LGRADU+NPRA),1)               
         END IF                                                         
         ...
         IF(ORBGRD.LT.SOGTOL  .OR.  ITSO.GT.0) THEN                     
            ...
            IF(ITSO.EQ.0) THEN                                          
              ...
              IF(SCFTYP.EQ.UHF) THEN                                    
                ...
                CALL XCOPY(NPRA,X(LHESSA),1,X(LHESSU)     ,1)           
                CALL XCOPY(NPRB,X(LHESSB),1,X(LHESSU+NPRA),1)           
              ELSE                                                      
...
C                                                                       
     ...
     *                     ORBGRD,NPRU,ITSO,NFT15,dum,.false.)          
               ...
               CALL XCOPY(NPRU,X(LGRADU),1,X(LPGRADU),1)                
               CALL XCOPY(NPRA,X(LDISPLU),1,X(LDISPLA),1)               
               CALL XCOPY(NPRB,X(LDISPLU+NPRA),1,X(LDISPLB),1)          
     ...
     *                     ORBGRD,NPRO,ITSO,NFT15,dum,.false.)          
               IF(ORBGRD.EQ.-123) GO TO 550                             
               CALL SOTRAN(X(LDISPLO),X(LVA),X(LWRK1),X(LSCR),          
     *                       NPRO,L0,L1,NA,NB,ORBGRD)                   
               CALL XCOPY(NPRO,X(LGRADO),1,X(LPGRADO),1)                
            END IF                                                      
            GO TO 400                                                   
         END IF                                                         
      END IF                                                            
...
C       HAS NO DFT EXCHANGE-CORRELATION YET IN THIS CASE OF FDIFF!      
        CALL DAREAD(IDAF,IODA,X(LWRK1),L2*2,317,0)                      
        CALL VADD(X(LWRK1   ),1,X(LFXCA),1,X(LWRK1   ),1,L2)            
        CALL VADD(X(LWRK1+l2),1,X(LFXCB),1,X(LWRK1+l2),1,L2)            
        CALL DAWRIT(IDAF,IODA,X(LWRK1),L2*2,317,0)                      
      END IF                                                            
C                                                                       
      IF(CVGED  .AND.  CCTYP.NE.ANONE) THEN                             
...
C                                                                       
      ...
      IF(MPLEVL.EQ.2  .AND.  OSPT.EQ.ZAPT .AND.                         
     *    CITYP.EQ.ANONE ) THEN                                         
...
C                                                                       
      IF((CITYP.EQ.SFCIS.OR.TDDFTYP.EQ.SPNFLP                           
     *     .OR.CITYP.EQ.SFORM.OR.CITYP.EQ.SFDET)                        
     *     .AND.SCFTYP.EQ.ROHF) THEN                                    
...
C                                                                       
      IF(ISVP.EQ.1  .AND.  MASPRT) THEN                                 
         ...
         CALL TRPOSE(X(LVA),X(LWRK1),L1,L1,1)                           
         IF(SCFTYP.EQ.UHF) CALL DAREAD(IDAF,IODA,X(LVB),L3,19,0)        
         IF(SCFTYP.EQ.UHF) CALL TRPOSE(X(LVB),X(LWRK2),L1,L1,1)         
         CALL SVPOUT(X(IGRDDFT),X(LVA),X(LVB),L1,L2,                    
     *               X(LDA),X(LDB),ETOT,ECHIP,NPRINT,'UHFOP ')          
         CALL TRPOSE(X(LVA),X(LWRK1),L1,L1,1)                           
         IF(SCFTYP.EQ.UHF) CALL TRPOSE(X(LVB),X(LWRK2),L1,L1,1)         
      END IF                                                            
C                                                                       
