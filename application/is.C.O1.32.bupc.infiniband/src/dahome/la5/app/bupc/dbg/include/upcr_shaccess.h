 ...
 *
 * $Id: upcr_shaccess.h,v 1.79 2012/01/04 20:49:28 phargrov Exp $
  ...
  extern int upcri_trace_suppresslocal;
    ...
    #define _UPCRI_TRACE_LOCAL_PUTGET(name,dest,src,nbytes) do {              \
      if (!upcri_trace_suppresslocal) {                                       \
        ...
        UPCRI_TRACE_PRINTF_NOPOS((#name ": sz = %6llu  dst=<%p>  src=<%p>%s", \
        _nbytes, (void *)(dest), _src, _data_tmp));                           \
      }                                                                       \
    } while (0)
  #else
...
/* upc_cast() family.  See upcr_preinclude/bupc_extensions.h */
GASNETT_INLINE(_bupc_cast)
void *_bupc_cast(upcr_shared_ptr_t sptr) {
  upcri_checkvalid_shared(sptr);
  return upcr_isnull_shared(sptr) ? NULL
          : upcr_shared_to_processlocal(sptr);
}
GASNETT_INLINE(_bupc_castable)
int _bupc_castable(upcr_shared_ptr_t sptr) {
  upcri_checkvalid_shared(sptr);
 #if UPCRI_BUPC_CAST_PROCESS_SCOPE
  return (upcri_s_nodeof(sptr) == gasnet_mynode()) || upcr_isnull_shared(sptr);
 #else
  return upcri_s_islocal(sptr) || upcr_isnull_shared(sptr);
 #endif
}
GASNETT_INLINE(_bupc_thread_castable)
int _bupc_thread_castable(unsigned int thr) {
 #if UPCRI_BUPC_CAST_PROCESS_SCOPE
  return (upcri_thread_to_node(thr) == gasnet_mynode());
 #else
  return upcri_thread_is_local(thr);
 #endif
}
		 ...
		 size_t nbytes, int isstrict UPCRI_PT_ARG)
{
    ...
    upcri_checkvalid_nonnull_shared(dest);
		  ...
		  size_t nbytes, int isstrict UPCRI_PT_ARG)
{
    ...
    upcri_checkvalid_nonnull_pshared(dest);
    UPCRI_TRACE_STRICT(isstrict);
    #define UPCRI_PEVT_ARGS , !isstrict, &UPCRI_PEVT_PTMP, src, nbytes
		 ...
		 size_t nbytes, int isstrict UPCRI_PT_ARG)
{
    ...
    upcri_checkvalid_nonnull_shared(src);
    UPCRI_TRACE_STRICT(isstrict);
		  ...
		  size_t nbytes, int isstrict UPCRI_PT_ARG)
{
    ...
    upcri_checkvalid_nonnull_pshared(src);
    UPCRI_TRACE_STRICT(isstrict);
    #define UPCRI_PEVT_ARGS , !isstrict, dest, &UPCRI_PEVT_PTMP, nbytes
...
void _upcr_wait_syncnb_strict(upcr_handle_t handle UPCRI_PT_ARG_IFINST) {
  #define UPCRI_PEVT_ARGS , handle
  upcri_pevt(GASP_UPC_NB_SYNC,{},{
    gasnet_wait_syncnb(handle);
...
#define upcr_wait_syncnb_strict(handle) \
       (upcri_srcpos(), _upcr_wait_syncnb_strict(handle UPCRI_PT_PASS_IFINST))
...
int _upcr_try_syncnb_strict(upcr_handle_t handle UPCRI_PT_ARG_IFINST) {
  int retval;
  #define UPCRI_PEVT_ARGS , handle, retval
  upcri_pevt(GASP_BUPC_NB_TRYSYNC,{},{
    retval = (gasnet_try_syncnb(handle) == GASNET_OK);
...
#define upcr_try_syncnb_strict(handle) \
       (upcri_srcpos(), _upcr_try_syncnb_strict(handle UPCRI_PT_PASS_IFINST))
...
_upcr_memput(upcr_shared_ptr_t dst, const void *src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    upcri_checkvalid_nonnull_shared(dst);
    #define UPCRI_PEVT_ARGS , &dst, src, nbytes
