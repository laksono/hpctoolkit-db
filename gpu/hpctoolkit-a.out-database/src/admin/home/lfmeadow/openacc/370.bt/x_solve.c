...
//  double temp1, temp2, temp3;
  ...
  //---------------------------------------------------------------------
//#pragma acc data present(rho_i,u,qs,rhs,square) pcreate(lhsX,fjacX,njacX) 
  struct timespec ts1, ts2;
  clock_gettime(CLOCK_MONOTONIC_RAW, &ts1);
#pragma omp target data map(alloc:lhsX,fjacX,njacX)
{
...
printf("x_solve alloc: %g ms\n", t0*1000);
#pragma omp target teams distribute parallel for collapse(2)\
  private(i,j,k,temp1,temp2,temp3)
  for (k = 1; k <= gp22; k++) {
    for (j = 1; j <= gp12; j++) {
      for (i = 0; i <= isize; i++) {
        temp1 = rho_i[k][j][i];
